...

{
...

    {
        ...
        return view('meetups.index', compact('assets'));
    ...
    { 
        ...
        $relations = [
            ...
            'sites' => \App\Sites::where('status', '=', '1')->get()->pluck('name', 'id')->prepend('Please select', ''),
        ...
        return view('meetups.create', $relations);
...

    {
        ...
        return view('meetups.edit', compact('task') + $relations);
    ...
    {        
        $task = Task::findOrFail($id);
        if(!$task) return redirect()->route('meetups.index')->withErrors('Task not found');
        ...
        if($uid<>1) {
            $taskUser = TaskUser::select("task_users.user_id")
                ...
                ->where('users.id',$uid)
                ->first();
            if(!$taskUser) return redirect()->route('meetups.index')->withErrors('You dont have access to view task: '.$task->tcode);
        }
        ...
        return view('meetups.show', $relations);
    ...
    public function saveTaskComment(Request $request){
        ...
        return redirect()->route('meetups.show',[$task_id]);
    }
