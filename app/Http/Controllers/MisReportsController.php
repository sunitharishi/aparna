...
use App\Flushmisreport;
use App\Flushconsumpmisreport;
...

{
		...
		$twrs_fr = array();
		 ...
		 if($resnoc_count > 0){
			...
			if($kk == 9) {$sarovar_fr =  $noc_f_res[$kk];}
			   if($kk == 12) {$cyber_fr =  $noc_f_res[$kk];}
		   ...
		   $fnmatchfields_fornoc = ['month' => $segment4, 'year' => $segment3, 'site' =>$kk]; 
		   ...
		   if($fnresnoc_count > 0){
		   		...
		   		$noc_f_res[$kk] = $fresnoc_cres->noc_info;
				if($kk == 9) {$sarovar_noc =  $noc_fn_res[$kk];}
			   if($kk == 12) {$cyber_noc =  $noc_fn_res[$kk];}
			   if($kk == 11) {$aura_noc =  $noc_fn_res[$kk];}
			   if($kk == 10) {$hill_aven_noc =  $noc_fn_res[$kk];}
			    if($kk == 5) {$hill_lake_noc =  $noc_fn_res[$kk];}
			    if($kk == 18) {$twrs_noc =  $noc_fn_res[$kk];}
				 if($kk == 15) {$sarovargran_noc =  $noc_fn_res[$kk];}
		 ...
		 // $pdf = PDF::loadView('misprints.firenocrenewals', $relations);  
		  ...
		  /*$pdf = PDF::loadView('misprints.nocdataprint2', $relations);  
          $pdf->save(storage_path('/pdffiles/nocdataprint2.pdf'));*/ 
		  		
		$pdfFile1Path = storage_path() . '/pdffiles/nocdataprint.pdf';
		$pdfFile2Path = storage_path() . '/pdffiles/nocdataprintbkp.pdf';		  		
		/*$pdfFile3Path = storage_path() . '/pdffiles/nocdataprint2.pdf';*/
	...
	public function snagindexold(Request $request)
	  ...
	  $segment2 = Request::segment(2);
       ...
       $relations = [
		...
		$reportdatefrom_val = "1"."-".$segment4."-".$segment3;
		  ...
		  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Metro_water_Report')->first();
		  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
		  else $mislist = "5,7,8,9,11,12,13,14,15,16,20";		  
		...
		$cur_mn_days = date("t",strtotime($lastDateOfMonth));
		
		
		/*echo "test1".$lastDateOfMonth;
		echo "test2".$reportdate_val;
		echo "test3".$reportdatefrom_val;
		...
		exit;*/
		  ...
		  $sites_count = \App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->count();
		  $sites_res_arr =\App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
		  $rmachine_total = \App\Pmsdailyreportvalidation::get()->pluck('rmachine_total', 'site');
		  $battery_total = \App\Pmsdailyreportvalidation::get()->pluck('ride_btotal', 'site');
		  }else{
		    $role_id = Auth::user()->role_id;
			$emp_id = Auth::user()->emp_id;
			$getemp_info = \App\Emp::where('id','=',$emp_id)->first();
		  ...
		  foreach($sites_res_arr as $ke =>$site_res_row)
		  {
			...
			$pwrfailure3 = 0;
		    $mincn = 0;
			$reportdatefrom_val = date("Y-m-d",strtotime($reportdatefrom_val));
			$reportdate_val = date("Y-m-d",strtotime($reportdate_val));
		  	$result = Pmsdailyreport::where('reporton', '>=', $reportdatefrom_val)->where('reporton', '<=', $reportdate_val)->where('site', '=', $ke)->get();
			foreach($result as $res)
			{
				$ride_nw += (int)$res->ride_nw;
				$battery_nw += (int)$res->ride_bnw;
				if($res->ride_acovered!="")
				{
					$ride_acovered = str_replace("SFT","",$res->ride_acovered);
					$ride_acovered = str_replace("sft","",$ride_acovered);
					
					if($ride_acovered!="Nil" && $ride_acovered>0)
					{
						$ride_area += intval($ride_acovered);
						//echo $ride_area."<br/>";
					}
				}
				//(int)$ride_acovered += (int)$ride_acovered;
				if($res->housekp_rideon_hrs!="")
				{
					$tarr = explode(":",$res->housekp_rideon_hrs);
					if(!empty($tarr))	
					{
						if($tarr[0]!="") $hour = $tarr[0];	
						if($tarr[1]!="") $min = $tarr[1];	
						$tmin = ($hour * 60) + $min;	
						$mincn = $mincn + $tmin;
					}
				}
			...
			$rideNw[$ke] = $ride_nw;
			$batterNw[$ke] = $ride_bnw;
			$rideArea[$ke] = $ride_area;
			$hRhrs[$ke] = $pwrfailure3;
		  }	
		  $relations = [  			
				'sites' => $siteattrname,
				'rmachine_total' => $rmachine_total,
				'reportfrom_on' => $reportdatefrom_val,
				'report_on' => $reportdate_val,
				'report_year' => $segment3,
				'report_month' => $segment4,
			...
			$pdf = PDF::loadView('misprints.rswmachinprint', $relations);
			$pdf->setPaper('A4', 'landscape');			  
			date_default_timezone_set('Asia/Kolkata');
			$today_date = date('d');
			$today_time = date("g:i a");
			...
			$fnam = 'ROS_machine_MIS_formt _'.$monthName."_".$today_date."_".$segment3."_".$today_time.".pdf";
			return $pdf->download($fnam);
		}  
		if($segment2 == '19'){
		...
		$reportdatefrom_val = "1"."-".$segment4."-".$segment3;
		  ...
		  $sites_res_arr =\App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
		  $flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
		  $garden_area = \App\Pmsdailyreportvalidation::get()->pluck('garden_area', 'site');
		  ...
		  if($sites_count > 0){
			...
			foreach($sites_res_arr as $ke =>$site_res_row){
				...
				->first();
				  ...
				  $boresres = (float)$boresres + (float)$getsiteMetro->wsp_bores; 
				  $tankeres = (float)$tankeres + (float)$getsiteMetro->wsp_tankers;
				  $inlet = (float)$inlet + (float)$getsiteMetro->stp_avg_inlet_water; 
				}   
		...
		if($res_count > 0){
			...
			// GET OTHER SITES:
			 ...
			 foreach($sites_res_arr as $ke =>$site_res_row){
				...
				if($occcn > 0){
				 $occres =  Flushmisreport::
				where('month', '=', $segment4)
				->where('year', '=', $segment3)
				->where('site', '=', $ke)
				->first();
				  $occres_fwusage = (float)$occres->flush_water_usage; 
				  if($occres_fwusage!="") $occres_fwusage = $occres_fwusage;
				  else $occres_fwusage = 0; 	
				  
				  $occres_gwcon = (float)$occres->garden_water_con; 
				  if($occres_gwcon!="") $occres_gwcon = $occres_gwcon;
				  else $occres_gwcon = 0; 	
				  
				  $occres_others = (float)$occres->others; 
				  if($occres_others!="") $occres_others = $occres_others;
				  else $occres_others = 0; 	
				  
				  $occres_etwater = (float)$occres->excess_treated_water; 
				  if($occres_etwater!="") $occres_etwater = $occres_etwater;
				  else $occres_etwater = 0; 				 
				}
				else
				{
					 $occres_fwusage = 0; 
				...
				$ositesfwusage[$ke] = $occres_fwusage;
				$ositesgwcon[$ke] = $occres_gwcon;
				$ositesothers[$ke] = $occres_others;
				$ositesetwater[$ke] = $occres_etwater;
			}
		...
		$reswtre_count = \App\Flushconsumpmisreport::where($matchfields_m)->count();
		if($reswtre_count > 0) {
		 $reswtre_res  = \App\Flushconsumpmisreport::where($matchfields_m)->first(); 
		 $additional_text = $reswtre_res['additional_info'];
		} 
		...
		$relations = [  
			...
			'average' => $avgcon,
			'occupancy' => $occupancyarr,
			'ositesfwusage' => $ositesfwusage,
			'ositesgwcon' => $ositesgwcon,
			'ositesothers' => $ositesothers,
			'ositesetwater' => $ositesetwater,
			'boresavg' => $borewellcon,
			'tankdavg' => $tankcon,
			'inletavg' => $inletcon,
			'borewellsnum' => $num_borewells,
			...
			'additionalinfo' => $additional_text,
				...
				$pdf = PDF::loadView('misprints.etwaterconsumptionprint', $relations);
				$pdf->setPaper('A4', 'landscape');       
		...
		/*	 $monthName = date('F', mktime(0, 0, 0, $segment4, 10));   
			...
			$monthName = date('F', mktime(0, 0, 0, $segment4, 10)); 
			 ...
			 $fnam = 'STPFlushWaterConsumption_'.$monthName."_".$today_date."_".$segment3."_".$today_time.".pdf";
            // return $pdf->download('metrowater.pdf'); //Download    
			  return $pdf->download($fnam);
			 ...
			 if(count($formfieldarray) > 0){
			     foreach($formfieldarray as $fieldarr){
				 	 $existing_records[$fieldarr->site] = array("id"=>$fieldarr->id,"guards"=>$fieldarr->guards,"l_guards"=>$fieldarr->l_guards,"h_guards"=>$fieldarr->h_guards,"supervisors"=>$fieldarr->supervisors,"aso"=>$fieldarr->aso,"so_num"=>$fieldarr->so_num,"nalla_gandla_hub"=>$fieldarr->nalla_gandla_hub,"hillpark_hub"=>$fieldarr->hillpark_hub);
							...
							//Last date of current month.
							$lastDateOfMonth = date("t", strtotime($dateString));
							$reportdate_val = $segment3."-".$segment4."-".$lastDateOfMonth;
					 ...
					 if($fieldarr->site == 5 || $fieldarr->site == 6 || $fieldarr->site == 7 || $fieldarr->site == 8 || $fieldarr->site ==10 || $fieldarr->site ==14 || $fieldarr->site ==17 || $fieldarr->site == 16 || $fieldarr->site == 18 || $fieldarr->site == 19) {
					 $exist_Sec['sites'][$fieldarr->site] = get_sitename($fieldarr->site);
					 $exist_Sec['guards'][$fieldarr->site] = $fieldarr->guards;
					 $exist_Sec['l_guards'][$fieldarr->site] = $fieldarr->l_guards;
					 $exist_Sec['h_guards'][$fieldarr->site] = $fieldarr->h_guards;
			...
			$formfieldarray = \App\Clubutilizationmisreport::where($matchfields)->get();
			 ...
			 if(count($formfieldarray) > 0){
			  ...
			  foreach($siteattrname as $kk => $siteval){
				...
				foreach($getavg_info_res as $rowres){
				$swimpool += (int)$rowres->clbhous_users_pool;
				$gym += (int)$rowres->clbhous_users_gym;
				$tennis += (int) $rowres->clbhous_users_tennis;
				if($rowres->clbhous_shuttle!="") $badminton += (int)$rowres->clbhous_shuttle;
				}
				}
				$day_cn=cal_days_in_month(CAL_GREGORIAN,$month,$year);
				...
				$av_occ = round((float)(($owners + $tenents)/2));
				$avg_swimpool_per = 0;
				$avg_gym_per = 0;$avg_tennis_per = 0;
				$avg_badminton_per = 0;
				if($av_occ > 0){
				$avg_swimpool_per = round(($avg_swimpool * 100)/($av_occ));
				$avg_gym_per = round(($avg_gym * 100)/($av_occ));
 ...
 $pdf->setPaper('A4', 'landscape');
	...
	//	return $pdf->download('traffic.pdf'); //Download  
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
		  ...
		  $flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
			...
			$sitearr = explode(",",$siteinfo);
			$sitenames = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id')->prepend('Please select', '');
			$siteattrname = \App\Sites::whereIn('id', $sitearr)->where('id', '!=', '12')->where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
			$flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
	...
	public function getnocformview(Request $request)
	{
	    ...
	    if(Auth::user()->id == 1){
	      $sitenames = \App\Sites::where('status', '=', '1')->where('flat_type', '=', 'flats')->where('noc', '=', 'Yes')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');
		  $siteattrname = \App\Sites::where('status', '=', '1')->where('flat_type', '=', 'flats')->where('noc', '=', 'Yes')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
		  $flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
			...
			$siteinfo = $getemp_info->community;
			$sitearr = explode(",",$siteinfo);
			$sitenames = \App\Sites::whereIn('id', $sitearr)->where('flat_type', '=', 'flats')->where('noc', '=', 'Yes')->where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');
			$siteattrname = \App\Sites::whereIn('id', $sitearr)->where('flat_type', '=', 'flats')->where('noc', '=', 'Yes')->where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
			$flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
		  }
	   ...
	   return view('misreportsdetail.nocsites', $relations);
	}
	  ...
	  $segment4 = Request::segment(4);
       ...
       $relations = [
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
		  ...
		  $occupan_count = \App\Metrowatermisreport::where($matchcomfields)->count();
		
		if($occupan_count > 0){
		
			$formfieldarray = \App\Metrowatermisreport::where($matchcomfields)->first();
		...
		$matchcomfields = ['month' => $segment4, 'year' => $segment3]; 
		  ...
		  $occupan_count = \App\Flushconsumpmisreport::where($matchcomfields)->count();
		
		if($occupan_count > 0){
		
			$formfieldarray = \App\Flushconsumpmisreport::where($matchcomfields)->first();
		...
		$relations = [
		  ...
		  return view('misfiles.flushmis', $relations);
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
	      ...
	      if(Auth::user()->id == 1){
	      $sitenames = \App\Sites::where('status', '=', '1')->where('flat_type', '=', 'flats')->orderBy('scode', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');
		  $siteattrname = \App\Sites::where('status', '=', '1')->where('flat_type', '=', 'flats')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
		  $flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
			...
			$siteinfo = $getemp_info->community;
			$sitearr = explode(",",$siteinfo);
			$sitenames = \App\Sites::whereIn('id', $sitearr)->where('flat_type', '=', 'flats')->where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');
			$siteattrname = \App\Sites::whereIn('id', $sitearr)->where('flat_type', '=', 'flats')->where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
			$flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
		  }
	  ...
	  $segment2 = Request::segment(2);
       ...
       $relations = [
		...
		$relations = [  
		  ...
		  return view('misfiles.view.etwaterconsumption', $relations);
			...
			$formfieldarray = \App\Securitymisreport::where($matchfields)->get();
			 ...
			 if(count($formfieldarray) > 0){
			     foreach($formfieldarray as $fieldarr){
					 ...
					 if($fieldarr->site == 8 || $fieldarr->site == 19 || $fieldarr->site == 14 || $fieldarr->site == 5 || $fieldarr->site == 7 || $fieldarr->site ==16 || $fieldarr->site ==17 || $fieldarr->site == 18) {
		...
		if($res_count > 0){
			...
			$formfieldarray = \App\Clubutilizationmisreport::where($matchfields)->get();
			 ...
			 foreach($siteattrname as $kk => $siteval){
				...
				$swimpool += $rowres->clbhous_users_pool;
				$gym += $rowres->clbhous_users_gym;
				$tennis += $rowres->clbhous_users_tennis;
				if($rowres->clbhous_shuttle!="") $badminton += $rowres->clbhous_shuttle;
				}
				}
				$day_cn=cal_days_in_month(CAL_GREGORIAN,$month,$year);
 ...
 // STORE Metro water Report
 
   public function storemetrowater(StoreBorewellnwRequest $request)
    ...
    {  
	 ...
	 if(isset($request['report_status'])) { $request['report_status'] = $request['report_status'];}
		 ...
		 $month = $request['month'];
		   ...
		   if((int)$request['record_id'] > 0){
		     $report = Metrowatermisreport::findOrFail($request['record_id']);
					...
					/* echo "<pre>";     
					 print_r($store_val);
					 echo "</pre>"; */ 
					  Metrowatermisreport::create($store_val); 
...
// STORE WATER CONSUMPTION
   ...
   public function storeflushconsump(StoreBorewellnwRequest $request)
    ...
    {  
	 ...
	 if(isset($request['report_status'])) { $request['report_status'] = $request['report_status'];}
		 ...
		 $month = $request['month'];
		   ...
		   if((int)$request['record_id'] > 0){
		     $report = Flushconsumpmisreport::findOrFail($request['record_id']);
					...
					/* echo "<pre>";     
					 print_r($store_val);
					 echo "</pre>"; */ 
					  Flushconsumpmisreport::create($store_val); 
 ...
 public function storeflush(StoreBorewellnwRequest $request)
    ...
    {  
		// $request['frecord_id'];
		 ...
		 $month = $request['month'];
		   ...
		   if((int)$request['frecord_id'] > 0){
		   	
		     $report = Flushmisreport::findOrFail($request['frecord_id']);

					   $store_val  = array('site'=>$request['site'],'flush_water_usage'=>$request['flush_water_usage'],'garden_water_con'=>$request['garden_water_con'],'others'=>$request['others'],'excess_treated_water'=>$request['excess_treated_water']);
				     ...
				     $store_val  = array('site'=>$request['site'], 'month'=>$request['month'], 'year'=>$request['year'], 'user_id'=>$request['user_id'],'flush_water_usage'=>$request['flush_water_usage'],'garden_water_con'=>$request['garden_water_con'],'others'=>$request['others'],'excess_treated_water'=>$request['excess_treated_water']);
					/* echo "<pre>";     
					 print_r($store_val);
					 echo "</pre>"; */ 
					  Flushmisreport::create($store_val); 
	...
	public function storenocfiresafe(StoreFiresafemisRequest $request)
	{
		$edata = $request->all();
		foreach($edata['blockname'] as $key=>$value)
		{
			$blockname = $value;
			if($blockname!="")
			{			
				$name_change_socity = $edata['name_change_socity'][$key];
				$valid_upto = $edata['valid_upto'][$key];
				$id = $edata['ids'][$key];
				if($id>0)
				{				
					$comment = Blocknocmonthreport::findOrFail($id);			
					$etinfo = [
						"site"=>$edata['site'],"valid_year"=>$edata['year'],"valid_month"=>$edata['month'],"blockname"=>$value,"name_change_socity"=>$name_change_socity,"valid_upto"=>$valid_upto
					];				
					$comment->update($etinfo); 
				}
				else
				{
					$diconsumptn = array("site"=>$edata['site'],"valid_year"=>$edata['year'],"valid_month"=>$edata['month'],"blockname"=>$value,"name_change_socity"=>$name_change_socity,"valid_upto"=>$valid_upto);
					$insertcon = Blocknocmonthreport::create($diconsumptn); 
				}
			}
		}
		return redirect('/firereports');
	}
 
   public function storemisallsites(StoreFiresafemisRequest $request)
   {
   	  $edata = $request->all();
	  $id = $edata['id'];
	  if($id>0)
	  {		$slist = implode(",",$edata['site']);		
			$comment = Misreportselectedsite::findOrFail($id);			
			$etinfo = [
				"reportname"=>$edata['report_name'],"slist"=>$slist
			];				
			$comment->update($etinfo); 
		}
		else
		{	
			$slist = implode(",",$edata['site']);	
			$diconsumptn = array("reportname"=>$edata['report_name'],"slist"=>$slist);
			$insertcon = Misreportselectedsite::create($diconsumptn); 
		}
		return redirect('/missiteselection');
   }
		   ...
		   if((int)$request['record_id'] > 0){
		     $report = Firesafetymisreport::findOrFail($request['record_id']);
					...
					// $firesafe_issue =  Firesafenotworkingissue::create($cat_arr);
					 ...
					 if(count($edata['category'])> 0){
					 	foreach($edata['category'] as $key => $cat){
						   $cat_arr =  array("category"=>$cat,"issue_des"=>$edata['issue_des'][$key],"root_cause"=>$edata['root_cause'][$key],"act_req_plan"=>$edata['act_req_plan'][$key],"pendingfromdays"=>$edata['pendingfromdays'][$key],"reponsibility"=>$edata['reponsibility'][$key],"notify_concern"=>$edata['notify_concern'][$key],"estimation_time"=>$edata['estimation_time'][$key],"ref_id"=>$request['record_id'],"site"=>$edata['site']);
						    $firesafe_issue =  Firesafenotworkingissue::create($cat_arr);
						}
					 }
					 }
				   ...
				   //  $store_val  = array('site'=>$request['site'], 'month'=>$request['month'], 'year'=>$request['year'],  'user_id'=>$request['user_id'], 'nw_bores_num'=>$request['nw_bores_num'], 'no_ground_water'=>$request['no_ground_water'],'over_load'=>$request['over_load'],'motor_brunt'=>$request['motor_brunt'],'cable_prblm'=>$request['cable_prblm'],'pumpormotorwear'=>$request['pumpormotorwear'],'others'=>$request['others'],'motor_brunt'=>$request['motor_brunt'],'dry_run_protectn'=>$request['dry_run_protectn'],'flow_meter'=>$request['flow_meter'],'remarks'=>$request['remarks'],'report_status'=>$request['report_status']);
					 ...
					 if(count($edata['category'])> 0){
					 	foreach($edata['category'] as $key => $cat){
						   $cat_arr =  array("category"=>$cat,"issue_des"=>$edata['issue_des'][$key],"root_cause"=>$edata['root_cause'][$key],"act_req_plan"=>$edata['act_req_plan'][$key],"pendingfromdays"=>$edata['pendingfromdays'][$key],"reponsibility"=>$edata['reponsibility'][$key],"notify_concern"=>$edata['notify_concern'][$key],"estimation_time"=>$edata['estimation_time'][$key],"ref_id"=>$firesafeid,"site"=>$edata['site']);
						    $firesafe_issue =  Firesafenotworkingissue::create($cat_arr);
						}
					 }
					 }
   ...
   public function storemisstp(StoreStpRequest $request)
    ...
    {  
	 ...
	 if(isset($request['report_status'])) { $request['report_status'] = $request['report_status'];}
		   ...
		   if(isset($request['record_id']) && (int)$request['record_id'] > 0){
...
public function missiteselection(Request $request)
{
	return view('misreportsdetail.allsiteselection');
}
...
/*----------------------------------REPORT ONE START (Metro Water Supply details)----------------------------------------------------*/
		...
		$reportdatefrom_val = "1"."-".$segment4."-".$segment3;
		  ...
		  $sites_res_arr =\App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');		  
			...
			$sitenames = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('Please select', '');
			$siteattrname = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
			$sites_count = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->orderBy('name', 'ASC')->count();
			$sites_res_arr =\App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
		  }	  
		  $fromdate = $segment3."-".$segment4."-"."1";
		  $todate = $segment3."-".$segment4."-".$lastdatearr[2];		  
		  $skarra= array();
		...
		$relations2 = [  			
			...
			$pdf = PDF::loadView('misprints.metrowaterdownload', $relations);
			$pdf->save(storage_path('/pdffiles/omismetrowaterdownload.pdf'));
			
			$im = new \Imagick();
            $im->pingImage(storage_path() . '/pdffiles/omismetrowaterdownload.pdf');
            $omispdfFile1Count =  $im->getNumberImages(); 
			if($omispdfFile1Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile1Count;
			
			/*----------------------------------REPORT ONE END (Metro Water Supply details)-----------------------------------------*/
			
			
			/*----------------------------------REPORT TWO START (WATER CONSUMPTION)-----------------------------------------*/
		...
		$reportdatefrom_val = "1"."-".$segment4."-".$segment3;
	      ...
	      $sitenames = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('All', '');
		  $siteattrname = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
		  $sites_count = \App\Sites::where('status', '=', '1')->orderBy('sname', 'ASC')->count();
		  $sites_res_arr =\App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id');
		  $flatres = \App\Pmsdailyreportvalidation::get()->pluck('flats', 'site');
		  ...
		  $skarra= array();
		  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Water_source_and_consumption')->first();
		  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
		  else $mislist = "9,15,13,11,8,5,14,7,16,17,20,12,87";		
		  $allsitesList = explode(",",$mislist);
		  $i=0;
		  foreach($sites_res_arr as $keys => $site){
		  	if (in_array($keys, $allsitesList)) 
		...
		$relations = [  
		    ...
		    $waterpcn = 2;
		  	$pdf = PDF::loadView('misprints.waterconsumptionprint', $relations);
			$pdf->setPaper('A4', 'landscape');
			$pdf->save(storage_path('/pdffiles/omiswaterconsumdownload.pdf'));

			$im = new \Imagick();
            $im->pingImage(storage_path() . '/pdffiles/omiswaterconsumdownload.pdf');
            $omispdfFile2Count =  $im->getNumberImages(); 
			if($omispdfFile2Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile2Count;
			
			/*----------------------------------REPORT TWO END (WATER CONSUMPTION)-----------------------------------------*/
			
			
			/*----------------------------------REPORT Four Start (BOREWELL NOTWORKING)-----------------------------------------*/
			...
			if($res_count > 0){
			  ...
			  $skarra= array();
			  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Borewells_not_working_status')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
			  else $mislist = "9,15,13,11,8,5,14,7,16,17,20,12,87";		  
			  $allsitesList = explode(",",$mislist);
			  $i=0;
			...
			// }
			 ...
			 $pdf = PDF::loadView('misprints.borewellmisprint', $relations);
			 $pdf->setPaper('A4', 'landscape');
			 $pdf->save(storage_path('/pdffiles/omisborewellnotworking.pdf')); 
			 
			 $im = new \Imagick();
             $im->pingImage(storage_path() . '/pdffiles/omisborewellnotworking.pdf');
             $omispdfFile4Count =  $im->getNumberImages(); 
			 if($omispdfFile4Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile4Count;
			...
			$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
			 ...
			 $skarra= array();
			 $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Fire_safety_status')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
			  else $mislist = "9,15,13,11,18,5,7,20,12";		  
			  $allsitesList = explode(",",$mislist);
			  $i=0;
			  foreach($sites_res_arr as $keys => $site){
				if (in_array($keys, $allsitesList)) 
			...
			$relations = [
							...
							if($tmispdfFilesCount%2!=0)
							{
								 ...
								 foreach ($issuecount as $ikey => $issuecn) 
								 {
								   ...
								   foreach ($issuecount as $ikey => $issuecn) 
								   {
								   	   ...
								   	   if($k!=$ikey) $pdf->addPDF(storage_path() .'/pdffiles/firesafe/mistrafficprint'.$ikey.'.pdf', 'all');
									   if(isset($issuet[$ikey])) {
							 ...
							 $pdf->merge('file', storage_path('/pdffiles/omisfiresafeequipment.pdf'));
			 ...
			 $im = new \Imagick();
             $im->pingImage(storage_path() . '/pdffiles/omisfiresafeequipment.pdf');
             $omispdfFile5Count =  $im->getNumberImages(); 
			 if($omispdfFile5Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile5Count;
			 
			 /*----------------------------------REPORT Five End (Fire Safety Equipment)-----------------------------------------*/
			
			
			 /*----------------------------------REPORT Six First (Fire_NOC_Renewals)-----------------------------------------*/
	   ...
	   $afnoc_info_text = "";
		 foreach($siteattrname as $kk =>$siteat) {
		 $matchfields_fornoc = ['valid_month' => $segment4, 'valid_year' => $segment3, 'site' =>$kk]; 
		 //echo '<pre>'; print_r($matchfields_fornoc); echo '</pre>';
		 $resnoc_count = \App\Blocknocmonthreport::where($matchfields_fornoc)->count();
		 //echo "test321".$resnoc_count;
		 if($resnoc_count > 0){
		     $resnoc_cres = \App\Blocknocmonthreport::where($matchfields_fornoc)->get();
		    $noc_f_res[$kk] = $resnoc_cres;
			if($kk == 9) {$sarovar_fr =  $noc_f_res[$kk];}
		   ...
		   $fmonth = $segment4;
		   $matchfields_m = ['month' => $fmonth, 'year' => $segment3, 'site' =>$kk]; 	
		   //echo "<pre>"; print_r($matchfields_m); echo "</pre>";
		   $reswtre_count = \App\Firesafetymisreport::where($matchfields_m)->count();
		   //Sarovar Grande = 15
		   if($reswtre_count > 0 && $kk==15) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 // echo "<pre>"; print_r($reswtre_res); echo "</pre>";
			 if($reswtre_res) $sgnoc_info_text = $reswtre_res['noc_info'];
			}
		     //exit;		

			//AURA FIRE NOC DETAILS = 11
			if($reswtre_count > 0 && $kk==11) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 if($reswtre_res) $afnoc_info_text = $reswtre_res['noc_info'];
			}
			//CYBER LIFE FIRE NOC DETAILS 12
			if($reswtre_count > 0 && $kk==12) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 if($reswtre_res) $clnoc_info_text = $reswtre_res['noc_info'];
			}
			//SAROVAR FIRE NOC DETAILS 9
			if($reswtre_count > 0 && $kk==9) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 if($reswtre_res) $sfnoc_info_text = $reswtre_res['noc_info'];
			}
			//HILLPARK LAKE BREEZE FIRE NOC DETAILS 5
			if($reswtre_count > 0 && $kk==5) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 if($reswtre_res) $hlnoc_info_text = $reswtre_res['noc_info'];
			}
			//TOWERS FIRE NOC DETAILS 18
			if($reswtre_count > 0 && $kk==18) {
			 $reswtre_res  = \App\Firesafetymisreport::where($matchfields_m)->first(); 
			 if($reswtre_res) $tfnoc_info_text = $reswtre_res['noc_info'];
			}
		}
		//$sarovar_fr = array();
		//$cyber_fr = array();
		...
		//$aura_fr = array();
			...
			'sarovarinfo' => $sfnoc_info_text,
			'cyberinfo' => $clnoc_info_text,
			'gradeinfo' => $sgnoc_info_text,
			'towerinfo' => $tfnoc_info_text,
			'lakebinfo' => $hlnoc_info_text,
			'aurainfo' => $afnoc_info_text,
		   ...
		   $fcount = 0;
			...
			$pdf = PDF::loadView('misprints.nocdataprint', $relations);  
          	$pdf->save(storage_path('/pdffiles/omisfirenoc1.pdf')); 		  
		  
		  	$pdf = PDF::loadView('misprints.nocdataprintbkp', $relations);  
          	$pdf->save(storage_path('/pdffiles/omisfirenoc2.pdf')); 
			
			 //echo "FIRE:".$tmispdfFilesCount."<br/>";
			 
			 $im = new \Imagick();
             $im->pingImage(storage_path() . '/pdffiles/omisfirenoc1.pdf');
             $omispdfFile6aCount =  $im->getNumberImages(); 
			 if($omispdfFile6aCount>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile6aCount;
			 $im = new \Imagick();
			 $im->pingImage(storage_path() . '/pdffiles/omisfirenoc2.pdf');
             $omispdfFile6bCount =  $im->getNumberImages(); 
			 if($omispdfFile6bCount>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile6bCount;
			 
			 //echo "NOC:". $tmispdfFilesCount;
			 
			 
			 /*----------------------------------REPORT Six End (Fire_NOC_Renewals)-----------------------------------------*/
					 ...
					 if (count($issuecount) > 0) {
					  ...
					  $fi = 0;
							...
							$fi = $fi + 1;
							 ...
							 if($tmispdfFilesCount%2!=0)
							 {
								 ...
								 foreach ($issuecount as $ikey => $issuecn) 
								 {
								   ...
								   foreach ($issuecount as $ikey => $issuecn) 
								   {
									   ...
									   if($k!=$ikey) $pdf->addPDF(storage_path() .'/pdffiles/equipment/mistrafficprint'.$ikey.'.pdf', 'all');
									   if(isset($issuet[$ikey])) {
							 ...
							 $pdf->merge('file', storage_path('/pdffiles/omiselectomechanical.pdf'));
							 ...
							 $wqupmentcn =  $fi;
							 $im = new \Imagick();
							 $im->pingImage(storage_path() . '/pdffiles/omiselectomechanical.pdf');
							 $omispdfFile7Count =  $im->getNumberImages(); 
							 if($omispdfFile7Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile7Count;
							
			/*----------------------------------REPORT Seven First (Equipment Status)-----------------------------------------*/
			
			
			
			/*----------------------------------REPORT Eight First (STP Status)-----------------------------------------*/
			...
			if($formfieldarray){
			 ...
			 $totalValues = array();
			  ...
			  if(count($refid) > 0){
			   ...
			   $pdf = PDF::loadView('misprints.stpstatus', $relations);  
			   //$pdf->save(storage_path('/pdffiles/omisstpstatus.pdf')); 
						...
						if($tmispdfFilesCount%2!=0)
						{
							 ...
							 foreach ($issuecount as $ikey => $issuecn) 
							 {
								   if($i==0)
								   {
									   ...
									   {
										   $k = $ikey;
										   $pdf->addPDF(storage_path() .'/pdffiles/stp/mistrafficprint'.$ikey.'.pdf', 'all');
										   $i++;
									   }
								   }
							   }
							   foreach ($issuecount as $ikey => $issuecn) 
							   {
								   ...
								   if($k!=$ikey) $pdf->addPDF(storage_path() .'/pdffiles/stp/mistrafficprint'.$ikey.'.pdf', 'all');
								   if(isset($issuet[$ikey])) {
									  
									   $pdf->addPDF(storage_path() .'/pdffiles/stp/mistrafficprintissue'.$ikey.'.pdf', 'all');
								   }
						 ...
						 $pathForTheMergedPdf_fire_firesafe = storage_path() . "/pdffiles/stp/finalresult.pdf";
						 
						 $pdf->merge('file', storage_path('/pdffiles/omisstpstatus.pdf'));
						 ...
						 $im = new \Imagick();
						 $im->pingImage(storage_path() . '/pdffiles/omisstpstatus.pdf');
						 $omispdfFile8Count =  $im->getNumberImages(); 
						 if($omispdfFile8Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile8Count;
			
			
			/*----------------------------------REPORT Eight Last (STP Status)-----------------------------------------*/
			
			/*----------------------------------REPORT NINE Start (WSP Status)-----------------------------------------*/
		...
		//	 }    
			 ...
			 $totalValues = array();
			  ...
			  if(count($refid) > 0){
				...
				$wsp_issue_Count = $issuecount;
				  ...
				  $pdf = PDF::loadView('misprints.wspstatus', $relations);  
				  //$pdf->save(storage_path('/pdffiles/omiswspstatus.pdf')); 
				  $fi = 0; 		
				  if (count($issuecount) > 0) {
					  $mc = 0;$issuet = array();
					  $fi = 0;
							...
							if($tmispdfFilesCount%2!=0)
							{
								 ...
								 foreach ($issuecount as $ikey => $issuecn) 
								 {
									   if($i==0)
									   {
										   ...
										   {
											   $k = $ikey;
											   $pdf->addPDF(storage_path() .'/pdffiles/wsp/mistrafficprint'.$ikey.'.pdf', 'all');
											   $i++;
										   }
									   }
								   }
								   foreach ($issuecount as $ikey => $issuecn) 
								   {
									   ...
									   if($k!=$ikey) $pdf->addPDF(storage_path() .'/pdffiles/wsp/mistrafficprint'.$ikey.'.pdf', 'all');
									   if(isset($issuet[$ikey])) {
										  
										   $pdf->addPDF(storage_path() .'/pdffiles/wsp/mistrafficprintissue'.$ikey.'.pdf', 'all');
									   }
							 ...
							 $pdf->merge('file', storage_path('/pdffiles/omiswspstatus.pdf'));
							 ...
							 $im = new \Imagick();
							 $im->pingImage(storage_path() . '/pdffiles/omiswspstatus.pdf');
							 $omispdfFile9Count =  $im->getNumberImages(); 
							 if($omispdfFile9Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile9Count;
			
			
			
			/*----------------------------------REPORT NINE End (WSP Status)-----------------------------------------*/
			
			
			/*----------------------------------REPORT TEN START (CMD & RMD)-----------------------------------------*/
			...
			$relations = [  
				...
				if($res_count > 0){
					...
					$formfieldarray = \App\Cmdmisreport::where($matchfields)->get();
					 ...
					 $occres_res = array();
					  ...
					  $skarra= array();
					  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'CMD_vs_RMD')->first();
					  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
					  else $mislist = "9,15,13,11,8,5,14,7,19,18,20,12,87,17";		  
					  $allsitesList = explode(",",$mislist);
					  $i=0;
				...
				if($res_data_count > 0){
					...
					$pdf->setPaper('A4', 'landscape');  
					$pdf->save(storage_path('/pdffiles/omiscmdrmd.pdf')); 

					$im = new \Imagick();
					$im->pingImage(storage_path() . '/pdffiles/omiscmdrmd.pdf');
					$omispdfFile10Count =  $im->getNumberImages(); 
					if($omispdfFile10Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile10Count;

			
			/*----------------------------------REPORT TEN End (CMD & RMD)-----------------------------------------*/
		    ...
		    $securepcn = 2;
		    $pdf = PDF::loadView('misprints.missecurityoneprint', $relations);  
		    $pdf->save(storage_path('/pdffiles/omisdailysecuritydata.pdf')); 
			
			
			$im = new \Imagick();
			$im->pingImage(storage_path() . '/pdffiles/omisdailysecuritydata.pdf');
			$omispdfFile11Count =  $im->getNumberImages(); 
			if($omispdfFile11Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile11Count;
			...
			$lastdatearr = explode("-",$lastDateOfMonth);
			$skarra= array();
			$mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Traffic_Movement')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
			  else $mislist = "9,15,13,11,8,5,14,7,19,18,20,12,87,17";		  
			  $allsitesList = explode(",",$mislist);
			  $i=0;
		...
		//	 }    
			...
			/*if($sites_count > 0){
			 ...
			 $exist_Sec = array();
				...
				$pdf = PDF::loadView('misprints.mistrafficprint', $relations);
				$pdf->setPaper('A4', 'landscape');
				$pdf->save(storage_path('/pdffiles/omistrafficmovement.pdf')); 
				
				
				$im = new \Imagick();
				$im->pingImage(storage_path() . '/pdffiles/omistrafficmovement.pdf');
				$omispdfFile12Count =  $im->getNumberImages(); 
				if($omispdfFile12Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile12Count;
			
			
			/*----------------------------------REPORT Twelve End (Traffic movement Data)-----------------------------------------*/
			
			/*----------------------------------REPORT Thirteen Start (HOUSEKEEPING Download)-----------------------------------------*/
			...
			$skarra= array();
			$mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Housekeeping_Report')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
			  else $mislist = "9,15,13,11,8,5,14,7,19,18,20,12,87,17";		  
			  $allsitesList = explode(",",$mislist);
			  $i=0;
			...
			if($res_count > 0){
			 ...
			 //  echo "<pre>"; print_r($existing_records);echo "</pre>";
			  ...
			  $pdf->save(storage_path('/pdffiles/omishousekeepingdownload.pdf')); 
					  
			  $im = new \Imagick();
			  $im->pingImage(storage_path() . '/pdffiles/omishousekeepingdownload.pdf');
			  $omispdfFile13Count =  $im->getNumberImages(); 
			  if($omispdfFile13Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile13Count;
		
		
			
			/*----------------------------------REPORT Thirteen End (HOUSEKEEPING Download)-----------------------------------------*/
			
			
			/*----------------------------------REPORT Fourteen End (HORTICULTURE Download)-----------------------------------------*/
		...
		if($res_count > 0){
		   ...
		   $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Horticulture_Pesticides')->first();
		  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
		  else $mislist = "9,15,13,11,8,5,14,7,19,18,20,12,87,17";		  
		  $allsitesList = explode(",",$mislist);
		  $i=0;
		...
		// } 
		  ...
		  $siteattrnamenew_one = array();
		  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Housekeeping_Report1')->first();
		  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
		  else $mislist = "9,15,13,11,19,20,87";		  
		  $allsitesList = explode(",",$mislist);
		  $i=0;
		    ...
		    $matchhuskpfields = ['month' => $segment4, 'year' => $segment3, 'site' => $sk];
			 if (in_array($sk, $allsitesList)) 
			 {
			    ...
			    $siteattrnamenew_two = array();
				  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Housekeeping_Report2')->first();
				  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
				  else $mislist = "18,8,5,14,7,12,87";		  
				  $allsitesList = explode(",",$mislist);
				  $i=0;
				  foreach($siteattrname as $keys => $site){
					if (in_array($keys, $allsitesList)) 
	...
	/*	 $monthName = date('F', mktime(0, 0, 0, $segment4, 10));   
			  ...
			  return $pdf->download($fnam);  */
			    ...
			    $pdf->setPaper('A4', 'landscape'); 
				  ...
				  $pdf->save(storage_path('/pdffiles/omisreportonhorticulture.pdf')); 
				  
				  $horticulpcn = 5;
			// END HORTICULTURE
			
			$im = new \Imagick();
			$im->pingImage(storage_path() . '/pdffiles/omisreportonhorticulture.pdf');
			$omispdfFile14Count =  $im->getNumberImages(); 
			if($omispdfFile14Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile14Count;
			
			
			/*----------------------------------REPORT Fourteen End (HORTICULTURE Download)-----------------------------------------*/
			
			/*----------------------------------REPORT Fifteen Start (Club Download)-----------------------------------------*/
			...
			$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
			  ...
			  $skarra= array();
			  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Club_House_Utilization_Data')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
		...
		//	 }     		  
			...
			// return view('misprints.clubhouseprint', $relations);
			 ...
			 if($omisclubhouseCount>0)
			 {
			   $pdf = PDF::loadView('misprints.clubhouseprint', $relations);
			   $pdf->setPaper('A4', 'landscape'); 
			   $pdf->save(storage_path('/pdffiles/omisclubhouseutilization.pdf')); 
			   $im = new \Imagick();
			   $im->pingImage(storage_path() . '/pdffiles/omisclubhouseutilization.pdf');
			   $omispdfFile15Count =  $im->getNumberImages(); 
			   if($omispdfFile15Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile15Count;
			}
			
			/*----------------------------------REPORT Fifteen End (Club Download)-----------------------------------------*/
			...
			$relations = [  
			  ...
			  $skarra= array();
			  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Occupancy_Data')->first();
			  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
			  else $mislist = "9,15,13,11,19,18,8,5,14,7,20,12,87,17";		  
			  $allsitesList = explode(",",$mislist);
			  $i=0;
			...
			if($omisoccupancyCount>0)
			{
			   $pdf = PDF::loadView('misprints.occupancyprint', $relations);
			   $pdf->setPaper('A4', 'landscape');
			   $pdf->save(storage_path('/pdffiles/omisoccupancy_rdetails.pdf')); 
			   $pdf->setPaper('A4', 'landscape'); 
			   $im = new \Imagick();
			   $im->pingImage(storage_path() . '/pdffiles/omisoccupancy_rdetails.pdf');
			   $omispdfFile16Count =  $im->getNumberImages(); 
			...
			if($res_count > 0){
				...
				$formfieldarray = \App\Indentedmisreport::where($matchfields)->get();
				 ...
				 $Recordcount =  array();
				  ...
				  $skarra= array();
				  $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Indented_Material_Status')->first();
				  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
				  else $mislist = "9,15,13,11,19,18,8,5,14,7,20,12,87,17";		  
				  $allsitesList = explode(",",$mislist);
				  $i=0;
				 ...
				 if($omismindentedCount > 0) 
				{
				   $pdf = PDF::loadView('misprints.materialindented', $relations);
				   $pdf->setPaper('A4', 'landscape'); 
				   $pdf->save(storage_path('/pdffiles/omismaterialindented.pdf')); 
				   $im = new \Imagick();
				   $im->pingImage(storage_path() . '/pdffiles/omismaterialindented.pdf');
				   $omispdfFile17Count =  $im->getNumberImages(); 
				   if($omispdfFile17Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile17Count;
			  	}
			/*----------------------------------REPORT Seventeen End (Indented Material Status)-----------------------------------------*/
			
			
			/*----------------------------------REPORT Eighteen First (APNACOMLEX COMPLAINT REORT)-----------------------------------------*/
			...
			if($res_count > 0){
				...
				$formfieldarray = \App\Occupancymisreport::where($matchfields)->get();
				 ...
				 $skarra= array();
				 $mis_sites_list = Misreportselectedsite::where('reportname', '=', 'Apna_Complex_Complaint_Report')->first();
				  if($mis_sites_list) $mislist = $mis_sites_list['slist'];		  
				  else $mislist = "9,15,13,11,19,18,8,5,14,7,20,12,87,17";		  
				  $allsitesList = explode(",",$mislist);
				  $i=0;
			...
			{
				$pdf = PDF::loadView('misprints.apnacomplex', $relations);
				$pdf->save(storage_path('/pdffiles/omisapnacomplex.pdf')); 
				$im = new \Imagick();
				$im->pingImage(storage_path() . '/pdffiles/omisapnacomplex.pdf');
				$omispdfFile18Count =  $im->getNumberImages(); 
				if($omispdfFile18Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile18Count;
			}
			
			
			/*----------------------------------REPORT Eighteen End (APNACOMLEX COMPLAINT REORT)-----------------------------------------*/
			
			
			/*----------------------------------REPORT Nineteen First (APNACOMLEX COMPLAINT REORT)-----------------------------------------*/
			...
			$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
			  ...
			  $existing_records = array();  
			   ...
			   {		
			   	   $pdf = PDF::loadView('misprints.mismockdrillprint', $relations);	   
			       $pdf->save(storage_path('/pdffiles/omismockdrillprint.pdf')); 			   
				   $im = new \Imagick();
				   $im->pingImage(storage_path() . '/pdffiles/omismockdrillprint.pdf');
				   $omispdfFile19Count =  $im->getNumberImages(); 
				   if($omispdfFile19Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile19Count;
			   }
			
			/*----------------------------------REPORT Nineteen First (APNACOMLEX COMPLAINT REORT)-----------------------------------------*/
			
			
			/*----------------------------------REPORT Twenty First (APNACOMLEX COMPLAINT REORT)-----------------------------------------*/
			...
			$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
			  ...
			  $existing_records = array();  
				...
				$omisfprepareCount = count($indentrep_Array);
				...
				/*echo "<pre>"; print_r($indentrep_Array); echo "</pre>";
				exit;*/
		    ...
		    {	
			   $pdf = PDF::loadView('misprints.misfireprepareprint', $relations);		   
			   $pdf->save(storage_path('/pdffiles/omisfireprepareprint.pdf')); 			   
			   $im = new \Imagick();
			   $im->pingImage(storage_path() . '/pdffiles/omisfireprepareprint.pdf');
			   $omispdfFile20Count =  $im->getNumberImages(); 
			   if($omispdfFile20Count>0) $tmispdfFilesCount = $tmispdfFilesCount+$omispdfFile20Count;
			}
			
			$overallNoc = 0;
			if($omispdfFile6aCount>0 || $omispdfFile6bCount>0)
			{
				$overallNoc = $overallNoc+$omispdfFile6aCount;
				$overallNoc = $overallNoc+$omispdfFile6bCount;
			}
			
			if($omispdfFile1Count>0) $omispdfFile1Count = $omispdfFile1Count; else $omispdfFile1Count = 0;
			if($omispdfFile2Count>0) $omispdfFile2Count = $omispdfFile2Count; else $omispdfFile2Count = 0;
			if($omispdfFile4Count>0) $omispdfFile4Count = $omispdfFile4Count; else $omispdfFile4Count = 0;
			if($omispdfFile5Count>0) $omispdfFile5Count = $omispdfFile5Count; else $omispdfFile5Count = 0;
			if($overallNoc>0) $overallNoc = $overallNoc; else $overallNoc = 0;
			if($omispdfFile7Count>0) $omispdfFile7Count = $omispdfFile7Count; else $omispdfFile7Count = 0;
			if($omispdfFile8Count>0) $omispdfFile8Count = $omispdfFile8Count; else $omispdfFile8Count = 0;
			if($omispdfFile9Count>0) $omispdfFile9Count = $omispdfFile9Count; else $omispdfFile9Count = 0;
			if($omispdfFile10Count>0) $omispdfFile10Count = $omispdfFile10Count; else $omispdfFile10Count = 0;
			if($omispdfFile11Count>0) $omispdfFile11Count = $omispdfFile11Count; else $omispdfFile11Count = 0;
			if($omispdfFile12Count>0) $omispdfFile12Count = $omispdfFile12Count; else $omispdfFile12Count = 0;
			if($omispdfFile13Count>0) $omispdfFile13Count = $omispdfFile13Count; else $omispdfFile13Count = 0;
			if($omispdfFile14Count>0) $omispdfFile14Count = $omispdfFile14Count; else $omispdfFile14Count = 0;
			if($omispdfFile15Count>0) $omispdfFile15Count = $omispdfFile15Count; else $omispdfFile15Count = 0;
			if($omispdfFile16Count>0) $omispdfFile16Count = $omispdfFile16Count; else $omispdfFile16Count = 0;
			if($omispdfFile17Count>0) $omispdfFile17Count = $omispdfFile17Count; else $omispdfFile17Count = 0;
			if($omispdfFile18Count>0) $omispdfFile18Count = $omispdfFile18Count; else $omispdfFile18Count = 0;
			if($omispdfFile19Count>0) $omispdfFile19Count = $omispdfFile19Count; else $omispdfFile19Count = 0;
			if($omispdfFile20Count>0) $omispdfFile20Count = $omispdfFile20Count; else $omispdfFile20Count = 0;
			...
			$relations = [
			'Metro_Water_Report' => $omispdfFile1Count,  
			'Water_Source_and_Consumption' => $omispdfFile2Count,
			'Borewells_Not_Working_Status' => $omispdfFile4Count,
			'Fire_Safety_Status' => $omispdfFile5Count+$overallNoc,
			'Electro_Mechanical_Equipment_Status' => $omispdfFile7Count,
			'STP_Status' => $omispdfFile8Count,
			'WSP_Status' => $omispdfFile9Count,
			'CMD_vs_RMD' => $omispdfFile10Count,
			'Security_Report' => $omispdfFile11Count,
			'Traffic_Movement' => $omispdfFile12Count,
			'Housekeeping_Report' => $omispdfFile13Count,
			'Horticulture_Report' => $omispdfFile14Count,
			'Club_House_Utilization_Data' => $omispdfFile15Count,
			'Occupancy_Data' => $omispdfFile16Count,
			'Indented_Material_Status' => $omispdfFile17Count,
			'Apna_Complex_Complaint' => $omispdfFile18Count,
			'Mismockdrillprint' => $omispdfFile19Count,
			'Fire_Safety_Preparedness' => $omispdfFile20Count,
			];  
			
			
			 
	
			$pdf = PDF::loadView('misprints.indexpage', $relations);
			...
			/*---------------------------------------------------------ATTACH ALL OVERALL MIS FILES-------------------------------------*/
				
			$pdfFilestartpage = storage_path() . '/pdffiles/startpage.pdf'; 
			$pdfFileindexpage = storage_path() . '/pdffiles/indexpage.pdf'; 
				
			if($omispdfFile1Count>0) $pdfFile1Path = storage_path() . '/pdffiles/omismetrowaterdownload.pdf'; 
			if($omispdfFile2Count>0) $pdfFile2Path = storage_path() . '/pdffiles/omiswaterconsumdownload.pdf'; 
			if($omispdfFile4Count>0) $pdfFile4Path = storage_path() . '/pdffiles/omisborewellnotworking.pdf'; 
			if($omispdfFile5Count > 0) $pdfFile5Path = storage_path() . '/pdffiles/omisfiresafeequipment.pdf'; 
			if($omispdfFile6aCount>0) $pdfFile6aPath = storage_path() . '/pdffiles/omisfirenoc1.pdf'; 
			if($omispdfFile6bCount > 0) $pdfFile6bPath = storage_path() . '/pdffiles/omisfirenoc2.pdf'; 
			if($omispdfFile7Count > 0) $pdfFile7Path = storage_path() . '/pdffiles/omiselectomechanical.pdf'; 
			if($omispdfFile8Count > 0) $pdfFile8Path = storage_path() . '/pdffiles/omisstpstatus.pdf'; 
			if($omispdfFile9Count > 0) $pdfFile9Path = storage_path() . '/pdffiles/omiswspstatus.pdf'; 
			if($omispdfFile10Count > 0) $pdfFile10Path = storage_path() . '/pdffiles/omiscmdrmd.pdf'; 
			if($omispdfFile11Count > 0) $pdfFile11Path = storage_path() . '/pdffiles/omisdailysecuritydata.pdf';
			if($omispdfFile12Count > 0) $pdfFile12Path = storage_path() . '/pdffiles/omistrafficmovement.pdf';
			if($omispdfFile13Count > 0) $pdfFile13Path = storage_path() . '/pdffiles/omishousekeepingdownload.pdf';
			if($omispdfFile14Count > 0) $pdfFile14Path = storage_path() . '/pdffiles/omisreportonhorticulture.pdf';
			if($omispdfFile15Count > 0) $pdfFile15Path = storage_path() . '/pdffiles/omisclubhouseutilization.pdf';
			if($omispdfFile16Count > 0) $pdfFile16Path = storage_path() . '/pdffiles/omisoccupancy_rdetails.pdf';
			if($omispdfFile17Count > 0) $pdfFile17Path = storage_path() . '/pdffiles/omismaterialindented.pdf'; 
			if($omispdfFile18Count > 0) $pdfFile18Path = storage_path() . '/pdffiles/omisapnacomplex.pdf';
			if($omispdfFile19Count > 0) $pdfFile19Path = storage_path() . '/pdffiles/omismockdrillprint.pdf';
			if($omispdfFile20Count > 0) $pdfFile20Path = storage_path() . '/pdffiles/omisfireprepareprint.pdf';
			
			$pdf = new \PDFMerger();
			
			/*---------------------------------------------------------COMBINE ALL OVERALL MIS FILES-------------------------------------*/	
			
			if($pdfFilestartpage!="") $pdf->addPDF($pdfFilestartpage, 'all');
			if($pdfFileindexpage!="") $pdf->addPDF($pdfFileindexpage, 'all');
					
			if($pdfFile1Path!="") $pdf->addPDF($pdfFile1Path, 'all');
			if($pdfFile2Path!="") $pdf->addPDF($pdfFile2Path, 'all');
			if($pdfFile4Path!="") $pdf->addPDF($pdfFile4Path, 'all');
			if($pdfFile5Path!="") $pdf->addPDF($pdfFile5Path, 'all');
			if($pdfFile6aPath!="") $pdf->addPDF($pdfFile6aPath, 'all');
			if($pdfFile6bPath!="") $pdf->addPDF($pdfFile6bPath, 'all');
			if($pdfFile7Path!="") $pdf->addPDF($pdfFile7Path, 'all');
			if($pdfFile8Path!="") $pdf->addPDF($pdfFile8Path, 'all');
			if($pdfFile9Path!="") $pdf->addPDF($pdfFile9Path, 'all');
			if($pdfFile10Path!="") $pdf->addPDF($pdfFile10Path, 'all');
			if($pdfFile11Path!="") $pdf->addPDF($pdfFile11Path, 'all');
			if($pdfFile12Path!="") $pdf->addPDF($pdfFile12Path, 'all');
			if($pdfFile13Path!="") $pdf->addPDF($pdfFile13Path, 'all');
			if($pdfFile14Path!="") $pdf->addPDF($pdfFile14Path, 'all');
			if($pdfFile15Path!="") $pdf->addPDF($pdfFile15Path, 'all');
			if($pdfFile16Path!="") $pdf->addPDF($pdfFile16Path, 'all');
			if($pdfFile17Path!="") $pdf->addPDF($pdfFile17Path, 'all');
			if($pdfFile18Path!="") $pdf->addPDF($pdfFile18Path, 'all');
			if($pdfFile19Path!="") $pdf->addPDF($pdfFile19Path, 'all');
			if($pdfFile20Path!="") $pdf->addPDF($pdfFile20Path, 'all');
 ...
 public function downloadoverallmisindex(Request $request)
	...
	ini_set('memory_limit', '256M');
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
		 ...
		 // if(Auth::user()->id == 1){
		  foreach($siteattrname as $keys => $site){
		    if($keys == 9 || $keys == 15 || $keys == 13 || $keys == 11 || $keys == 18 || $keys == 5 || $keys == 7 || $keys == 20 || $keys == 12){
			   $skarra[0] = 9;
			   $skarra[1] = 15;
			   $skarra[2] = 13;
			   $skarra[3] = 11;
		...
		$twrs_fr = array();
		 foreach($siteattrname as $kk =>$siteat) {		 
		 if(isset($segment4) && $segment4!="") $segment4 = $segment4;
		 else $segment4 = 3;
		 $matchfields_fornoc = ['valid_month' => $segment4, 'valid_year' => $segment3, 'site' =>$kk]; 
		 $resnoc_count = \App\Blocknocmonthreport::where($matchfields_fornoc)->count();
		 if($resnoc_count > 0){
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
		 ...
		 if(Auth::user()->id == 1){
		  ...
		  //if(Auth::user()->id == 1){
		  foreach($siteattrname as $keys => $site){
		    if($keys == 9 || $keys == 15 || $keys == 13 || $keys == 11 || $keys == 8 || $keys == 5 || $keys == 14 || $keys == 7 || $keys == 19 || $keys == 18 || $keys == 20 || $keys == 12 || $keys == 87 || $keys == 17 ){
			   $skarra[0] = 9;
			   $skarra[1] = 15;
			   $skarra[2] = 13;
			   $skarra[3] = 11;
		...
		DB::enableQueryLog();
		if(Auth::user()->id == 1){
		//echo '<pre>';print_r($sitearrnew);echo '</pre>';


		$sitearr_new = implode(",",$sitearrnew);
		$query_eqp = "select * from `equipmentmisreports` where `site` in (".$sitearr_new.") and (`month` = ".$segment4." and `year` = ".$segment3.") and `equipmentmisreports`.`deleted_at` is null";		
		$formfieldarray = DB::select($query_eqp);

		//$res_count = \App\Equipmentmisreport::whereIn('site', $sitearrnew)->where($matchfields)->get();
		//echo '<pre>';print_r($res_count21);echo '</pre>';
		//$res_count2 = \App\Equipmentmisreport::whereIn('site', $sitearrnew)->where($matchfields);
		//$s = $res_count2->toSql();
		//dd($s);
		//exit;
		...
		//  if(Auth::user()->id == 1){
		  foreach($siteattrname as $keys => $site){
		    if($keys == 9 || $keys == 15 || $keys == 13 || $keys == 11 || $keys == 8 || $keys == 5 || $keys == 14 || $keys == 7 || $keys == 18 || $keys == 19 || $keys == 17 || $keys == 20 || $keys == 12 || $keys == 16 || $keys == 87){
			   $skarra[0] = 9;
			   $skarra[1] = 15;
			   $skarra[2] = 13;
			   $skarra[3] = 11;
		...
		//exit;
		$sitearr_new = implode(",",$sitearrnew);
		$query_stp = "select * from `stpmisreports` where `site` in (".$sitearr_new.") and (`month` = ".$segment4." and `year` = ".$segment3.") and `stpmisreports`.`deleted_at` is null";		
		$formfieldarray = DB::select($query_stp);
		if(count($formfieldarray) > 0){
		...
		$reportdate_val = $lastdatearr[2]."-".$segment4."-".$segment3;
		 ...
		 // if(Auth::user()->id == 1){
		  foreach($siteattrname as $keys => $site){
		    if($keys == 9 || $keys == 15 || $keys == 13 || $keys == 11 || $keys == 8 || $keys == 5 || $keys == 14 || $keys == 7 || $keys == 16 || $keys == 19 || $keys == 17 || $keys == 20 || $keys == 12 || $keys == 87){
			   $skarra[0] = 9;
			   $skarra[1] = 15;
			   $skarra[2] = 13;
			   $skarra[3] = 11;
	...
	//	 }    
		...
		$query_wsp = "select * from `wspmisreports` where `site` in (".$sitearr_new.") and (`month` = ".$segment4." and `year` = ".$segment3.") and `wspmisreports`.`deleted_at` is null";		
		$formfieldarray = DB::select($query_wsp);
       ...
       $relations = [
		...
		$reportdatefrom_val = "1"."-".$segment4."-".$segment3;
		  ...
		  $sites_res_arr =\App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
			...
			$siteinfo = $getemp_info->community;
			$sitearr = explode(",",$siteinfo);
			$sitenames = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->where('id', '!=', '95')->orderBy('scode', 'ASC')->get()->pluck('name', 'id')->prepend('Please select', '');
			$siteattrname = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->where('id', '!=', '95')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
			$sites_count = \App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->where('id', '!=', '95')->orderBy('scode', 'ASC')->count();
			$sites_res_arr =\App\Sites::whereIn('id', $sitearr)->where('status', '=', '1')->where('id', '!=', '95')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
		  }
