...
class CommunityAssetsController extends Controller
{
	 ...
	 public function assetReport(Request $request)
    {
        ...
        if($c) $assets = $assets->where('community_assets.site_id', $c);
        if($ast) $assets = $assets->where('community_assets.category_id', $ast);
		if($asttype) $assets = $assets->where('community_assets.amc_type', $asttype);
		if($asttype=='amc' && $year!="") $assets = $assets->whereBetween('community_assets.amc_enddate',[$startdate, $enddate]);
		else if($asttype=='ppm'  && $year!="") $assets = $assets->whereBetween('community_assets.ppm_startdate', [$startdate, $enddate]);
		else if($asttype=='waranty'  && $year!="") $assets = $assets->WhereBetween('community_assets.waranty_enddate', [$startdate, $enddate]);
        $assets = $assets->orderBy('community_assets.id','desc')->get();
		
		$types = array(''=>'Select Type','amc'=>'AMC','waranty'=>'Waranty','ppm'=>'PPM');

        $sno=1;
        return view('community_assets.report', compact('assets','sno','categories','communities','types'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('community_assets.edit', $relations+compact('comm_asset','asset','template','sections','field_types','field_required','field_valids','assetTemplateForm','asset_template','amc_intervals','sitename','catgname','vendors','amc_type','ppm_type'));
    }
...

    {
		...
		$cassets = \App\Communityassetmaintenance::where('cam_id',$id)->get();
		
		$cassets = \App\Communityassetmaintenance::select("communityassetmaintenances.*",'sites.name as sitename')
	            ->leftJoin('sites','sites.id','=','communityassetmaintenances.site_id')
	            ->where('communityassetmaintenances.cam_id','=',$id)
				->orderBy('communityassetmaintenances.status', 'desc')
	            ->get();
...
$hctypes = array(1=>'Break Down',2=>'Incident Report',3=>'Break Down',4=>'Incident Report',5=>'AMC',6=>'New');
        return view('community_assets.show', compact('comm_asset','asset','template','sections','field_types','field_required','field_valids','asset_template','amc_intervals','sitename','catgname','vendorname','assetcode','historycards','jobcards','incidents','breakdowns','jbtypes','hctypes','amc_type','cassets'));
	...
	/*if($eRow->amc_startdate!="" || $eRow->amc_startdate!="0000-00-00") $startdate = $eRow->amc_startdate;
	 ...
	 public function communityassetTemplateedit(Request $request)
	 {
	 	$edata = $request->all();
		$eRow = Communityassetmaintenance::where('id', $edata['cassetId'])->first();
		if($eRow) {
	 ...
	 public function assetTemplatemaintenanceupdate(Request $request)
	 {
		...
		$eRow = Communityassetmaintenance::where('id', $id)->first();
		...
		//Communityassetmaintenance::create($etData);
	 }
	 
	 public function assetAlertEmails(Request $request)
	 {
	 	 $tomorrow = date("Y-m-d", strtotime("+1 day"));/*
		 $eRow = Communityassetmaintenance::where('alert_date', '=', $tomorrow)->where('status',1)->get();
		 if(count($eRow)>0)
		 {
		 	foreach($eRow as $key=>$assets)
			{
				$eRow = Communityassetmaintenance::where('id', $assets->id)->first();
			}
		 }*/
		 $start = date('Y-m-01');
		 $end = date('Y-m-t');
		 $eRow1 = Communityassetmaintenance::select("communityassetmaintenances.*",'categories.name as catgname','vendors.name as vendorname','assets.name as assetname','sites.name as sitename')->leftJoin('community_assets','community_assets.id','=','communityassetmaintenances.cam_id')->leftJoin('categories','categories.id','=','communityassetmaintenances.category_id')->leftJoin('vendors','vendors.id','=','community_assets.vendor')->leftJoin('assets','assets.id','=','community_assets.asset_id')->leftJoin('sites','sites.id','=','communityassetmaintenances.site_id')->where('communityassetmaintenances.status','=',1)->whereBetween('alert_date', [$start, $end])->where('communityassetmaintenances.status',1)->whereIn('communityassetmaintenances.amc_type',['amc','waranty'])->groupBy('communityassetmaintenances.id')->orderBy('communityassetmaintenances.alert_date', 'asc')->get();	
		 
		/* 
		  $eRow1 = DB::select( DB::raw("select id,name,alert_date,amc_startdate,amc_enddate,waranty_startdate,waranty_enddate,amc_type,status from communityassetmaintenances where alert_date >= '".$nextmonthStartDate."' and alert_date <= '".$nextmonthEndDate."' and status=1 and (amc_type='amc' OR amc_type='waranty')") );*/
	 ...
	 public function assetSMS(Request $request)
	 {
		...
		{
			$sites =  DB::select( DB::raw("select id,name,alert_date,amc_startdate,amc_enddate,waranty_startdate,waranty_enddate,amc_type,status from communityassetmaintenances where alert_date >= '".$start."' and alert_date <= '".$end."' and site_id=".$ke." and status=1  and (amc_type='amc' OR amc_type='waranty')") );			
	 ...
	 {
	 	/*$eRow = Communityassetmaintenance::get();
		if(count($eRow)>0)
		{
			$i=0;
			foreach($eRow as $key=>$assets)
		...
		$edata = $request->all();
		$asset_id = $edata['asset_id'];
		$assetRow = CommunityAsset::where('id',$asset_id)->first();		
		$amc_startdate = "NULL";	
		$amc_enddate = "NULL";	
		$waranty_startdate = "NULL";	
		$waranty_enddate = "NULL";	
		...
		$eRow = Communityassetmaintenance::where('cam_id', $asset_id)->first();
		if($eRow)
		{
			DB::table('communityassetmaintenances')->where("communityassetmaintenances.cam_id", '=',  $asset_id)->update(['communityassetmaintenances.status'=> 0]);			
		}
		...
		$cassetid = Communityassetmaintenance::create($etData);
    ...
    public function update(Request $request, $id)
    {
        ...
        if(isset($_FILES['asset_image']['name'])) {
		 ...
		 // SOP
          ...
          //  if($aiext=="jpg" || $aiext=="jpeg" || $aiext=="png" || $aiext=="gif") {
                $filename = uniqid()."_".$afval;
                $filepath = public_path().'/uploads/communityassets/'.$filename;
