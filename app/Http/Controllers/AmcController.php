...

    {
       ...
       //print_r($monthnames);print_r($months);
        ...
        foreach($categories as $ci=>$cval) {
			...
			$asst = CommunityAsset::select("name",'amc_type','amc_interval','amc_startdate','ppm_startdate','asset_group','asset_id')->where('site_id',$site_id)->where('category_id',$ci)->where('amc_interval','>',0);
			
            if($asst->count()) {
               //$assets = $asst->get();
			    
				$assets = $asst->groupBy('asset_group')->get();
                //$cassets[$ci] = array('assets'=>$assets,'rows'=>$asst->count());
				$cassets[$ci] = array('assets'=>$assets,'rows'=>$assets->count());
