...

{
...

    {
	    ...
	    if(isset($edata['smoke_image']))	{
		 ...
		 if(isset($edata['transfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['transcapacity'][$ckey],"filename"=>'','make'=>$edata['transmake'][$ckey]);   
		 ...
		 if(isset($edata['dgfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['dgcapacity'][$ckey],"filename"=>'','make'=>$edata['dgmake'][$ckey]);   
		...
		$diconsumptn = array();
		 ...
		 if(isset($edata['stpfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['stpcapacity'][$ckey],"filename"=>'','make'=>$edata['stpmake'][$ckey],'technology'=>$edata['stptechnology'][$ckey],'type'=>'single');   
		  ...
		  if(isset($edata['stpphcapacity'])) {
		 if(count($edata['stpphcapacity'] > 0)){
		 if(isset($edata['stpfilename'])) {
		 	   foreach($request->file('stpfilename') as $ckey => $consuption){
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['stpphcapacity'][$ckey],"filename"=>'','make'=>$edata['stpphmake'][$ckey],'technology'=>$edata['stpphtechnology'][$ckey],'type'=>'two');   
					  $insertcon = Sitestp::create($diconsumptn); 
					  $Transid = $insertcon->id;
					  if(isset($edata['stpphfilename'][$ckey])) {
						$extension = $consuption->getClientOriginalExtension();
						$file = $consuption->move('stps',$filename."_"."stp_pht".$Transid.".".$extension);
						$filename_extension = 'stps/'.$filename."_"."stp_pht".$Transid.".".$extension;
						$req_array =  array("filename"=> $filename_extension);
						$report = Sitestp::findOrFail($Transid);
						$report->update($req_array);
						}
 ...
 //WSP
		 ...
		 if(isset($edata['wspfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['wspcapacity'][$ckey],"filename"=>'','make'=>$edata['wspmake'][$ckey],'type'=>'single');   
	...
	// WSP PHASE TWO	 
		  ...
		  if(isset($edata['wspphcapacity'])) {
		 if(count($edata['wspphcapacity'] > 0)){
		 if(isset($edata['wspphfilename'])) {
		 	   foreach($request->file('wspphfilename') as $ckey => $consuption){
			   
			 
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['wspphcapacity'][$ckey],"filename"=>'','make'=>$edata['wspphmake'][$ckey],'type'=>'two');   
					  $insertcon = Sitewsp::create($diconsumptn); 
					  $Transid = $insertcon->id;
					  if(isset($edata['wspphfilename'][$ckey])) {
						$extension = $consuption->getClientOriginalExtension();
						$file = $consuption->move('wsps',$filename."_"."wsp_pht".$Transid.".".$extension);
						$filename_extension = 'wsps/'.$filename."_"."wsp_pht".$Transid.".".$extension;
						$req_array =  array("filename"=> $filename_extension);
						$report = Sitewsp::findOrFail($Transid);
						$report->update($req_array);
						}
		 ...
		 if(isset($edata['solarpwrfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['solarpwrcapacity'][$ckey],"filename"=>'','make'=>$edata['solarpwrmake'][$ckey]);   
     ...
     if(isset($edata['gacapacity'])) {
		 if(count($edata['gacapacity'] > 0)){
		  if(isset($edata['gafilename'])) { 
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['gacapacity'][$ckey],"filename"=>'',"make" => $edata['gamake'][$ckey]);   
...
echo "<pre>";
...

    {
	   ...
	   $edata['smoke_image'] = "";
		...
		$diconsumptn = array();
		 ...
		 // DB::table('sitehydrons')->where('site', '=', $id)->delete();
		 // DB::table('sitesolars')->where('site', '=', $id)->delete();
		 // DB::table('sitepwrsolars')->where('site', '=', $id)->delete();
		 // DB::table('sitepbas')->where('site', '=', $id)->delete();
		 // DB::table('sitefirealars')->where('site', '=', $id)->delete();	  
		...
		$diconsumptn = array();
		 ...
		 if(isset($edata['stpfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['stpcapacity'][$ckey],"filename"=>'','make'=>$edata['stpmake'][$ckey],'technology'=>$edata['stptechnology'][$ckey]);   
					  $insertcon = Sitestp::create($diconsumptn); 
					  $Transid = $insertcon->id;
					  if(isset($edata['stpfilename'][$ckey])) {
						$extension = $consuption->getClientOriginalExtension();
 ...
 //WSP
		 ...
		 if(isset($edata['wspfilename'])) {
			   ...
			   $filename = uniqid();
			   		if($consuption){ 
					  $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['wspcapacity'][$ckey],"filename"=>'','make'=>$edata['wspmake'][$ckey]);   
					  $insertcon = Sitewsp::create($diconsumptn); 
					  $Transid = $insertcon->id;
					  if(isset($edata['wspfilename'][$ckey])) {
						$extension = $consuption->getClientOriginalExtension();
		 ...
		 if(isset($edata['wspcapacity'])) {
		 	   foreach($edata['wspcapacity'] as $ckey => $consuption){
				 ...
				 $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['wspcapacity'][$ckey],'make'=>$edata['wspmake'][$ckey]); 
		...
		/*   if(isset($edata['num_of_pumps'])) {
		 if(count($edata['num_of_pumps'] > 0)){
		 if(isset($edata['num_of_pumps'])) {
		 	   foreach($edata['num_of_pumps'] as $ckey => $consuption){
			   	 if($edata['hy_id'][$ckey] > 0) {
				  
				 $report = Sitehydron::findOrFail($edata['hy_id'][$ckey]);
				 
				 $diconsumptn = array("site"=>$last_insert_id,"num_of_pumps"=>$edata['num_of_pumps'][$ckey],"capacity"=>$edata['hydrocapacity'][$ckey],'make'=>$edata['hydromake'][$ckey]); 
				 $report->update($diconsumptn);
				  $filename = uniqid();
				     if(isset($edata['hydrofilename'][$ckey])) {
					    $req = $edata['hydrofilename'][$ckey];
					    $Transid = $edata['hy_id'][$ckey];
						$extension = $req->getClientOriginalExtension();
						$file = $req->move('hydro',$filename."_"."hydro".$Transid.".".$extension);
						$filename_extension = 'hydro/'.$filename."_"."hydro".$Transid.".".$extension;
						$req_array =  array("filename"=> $filename_extension);
						$report = Sitehydron::findOrFail($Transid);
						$report->update($req_array);
		 ...
		 if(isset($edata['solarcapacity'])) {
		 	   foreach($edata['solarcapacity'] as $ckey => $consuption){
				 ...
				 $diconsumptn = array("site"=>$last_insert_id,"capacity"=>$edata['solarcapacity'][$ckey],'make'=>$edata['solarmake'][$ckey]); 
				 $report->update($diconsumptn);
				  $filename = uniqid();
				     if(isset($edata['solarfilename'][$ckey])) {
