...
use App\Http\Requests\RidePmsRequest;
...

{
	...
	$segment4 = Request::segment(4);
		...
		/*if($segment3 == '5')
		{
			...
			$lmatchfields = ['site' => $siteid]; 
			$locations = \App\Horticuture::where($lmatchfields)->get();
			...
			if($res_count > 0)
			{
				...
				$matchValidFields = ['site' => $siteid]; 
				$valid_count = \App\Horticuturedailyreportvalidation::where($matchValidFields)->count();		
				if($valid_count > 0){
					$validres = \App\Horticuturedailyreportvalidation::where($matchValidFields)->first();
				}
				$relations = [
				'client_statuses' => \App\DailyreportStatus::get()->pluck('title', 'id')->prepend('Please select', ''),
				'sites_names' => \App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id')->prepend('All', ''),
				...
				return view('edithorticulture', $relations);
			}
			return view('horticulture', $relations);
		}*/
      ...
      //exit;
       ...
       $matchprevfields =  ['site' => $siteid, 'reporton' => $prev_date]; 
		...
		$ridepmatchfields = ['site' => $siteid];

		$riderescn = \App\Pmsdailyreportvalidation::where($ridepmatchfields)->first();
		
		//echo "<pre>"; print_r($riderescn); echo "</pre>";

		if($riderescn) 
		{
			$rmachines =  explode(",",$riderescn->ride_machines);
			if(count($rmachines)>0)
			{
				foreach($rmachines as $rmachine)
				{
					$amatchfields = ['id' => $rmachine];
					$assetRes = \App\AparnaAsset::where($amatchfields)->first();
					if($assetRes)
					{
						$rideassetNames[] = $assetRes->code;
					}
				}
			}
	...
	if($segment3 == '6')
	{
		...
		$waterRes = array();
		$deppmatchfields = ['site' => $siteid];
		$deprescn = \App\Horticuturedailyreportvalidation::where($deppmatchfields)->first();
		if($deprescn) $flats =  $deprescn->flats;
		else $flats = "";
		$reportdate = date("Y-m-d");
		$reportdate = $segment4;
		...
		$lmatchfields = ['horticultures.community_id' => $siteid]; 		
		$locations =  \App\Horticulture::where($lmatchfields)->get();
		
		$locations =  Horticulture::select('horticultures.*','blocks.block_name')->leftJoin('blocks','blocks.id','=','horticultures.block_id')->where($lmatchfields)->groupBy('horticultures.sub_location','horticultures.block_id')->orderBy('blocks.block_name', 'ASC')->get();
		...
		if(count($locations)>0)
		{
			...
			{
				if($location->block_name) $blockName = $location->block_name."-";
				else $blockName = "";
				$locationsRes[$location->id] = $blockName.$location->sub_location;
			}
		}
			
		$prev_count = \App\Horticuturedailyreport::where($matchprevfields)->count();
		if($prev_count > 0)
		{
		  $formfieldprevarray = \App\Horticuturedailyreport::where($matchprevfields)->first();
		  $prev_apms_pending = $formfieldprevarray->apna_apms_pending;
		  $prev_project_pending = $formfieldprevarray->apna_project_pending; 
		}
		$matchValidFields = ['site' => $siteid]; 
		...
		if($res_count > 0)
		{
			$reportRes =  \App\Horticuturedailyreport::where($matchfields)->first();
			$rid = $reportRes->id;
			$formfieldarray = \App\Horticuturedailyreport::where($matchfields)->first();
			$siteid = $segment2;
			...
			if($water_count > 0)
			{
			  ...
			  {
			  	$waterRes[$wkey]['id'] = $waterres->id;
				$waterRes[$wkey]['name'] = $waterres->name;
				$waterRes[$wkey]['location'] = $waterres->location;
...
//return view('underconstruction', $relations);
    ...
    { 
	 ...
	 public function rideonpms(RidePmsRequest $request)
	 {
	 	$edata = $request->all();
		$nw = 0;
		
		if(isset($edata['ride_mname']) && count($edata['ride_mname'] > 0))
		{
			foreach($edata['ride_mname'] as $ckey => $rideon)
			{
				
				  $rdate = date("Y-m-d", strtotime($edata['rdate']));				  
				  $rideonqry = array("site"=>$edata['rsiteid'],"rdate"=>$rdate,'ride_mname'=>$edata['ride_mname'][$ckey],'ride_location'=>$edata['ride_location'][$ckey],'ride_rhrs'=>$edata['ride_rhrs'][$ckey],'ride_acovred'=>$edata['ride_acovred'][$ckey],'ride_nw'=>$edata['ride_nw'][$ckey],'ride_spares'=>$edata['ride_spares'][$ckey],'ride_btotal'=>$edata['ride_btotal'][$ckey],'ride_bhrs'=>$edata['ride_bhrs'][$ckey],'ride_bnw'=>$edata['ride_bnw'][$ckey],'ride_breason'=>$edata['ride_breason'][$ckey]); 
				  $insertcon = Rideonpms::create($rideonqry)->id;
				  if($insertcon>0)
				  {
				  	$nw = $nw+$edata['ride_nw'][$ckey];
				  } 						  		
			}
		}
		echo $nw;
	 ...
	 public function storehorti(StorePmsRequest $request)
	 {
	 	  $edata = $request->all();
		  $site = $edata['site'];
		  ...
		  if(isset($request['record_id']))
		  {
			 ...
			 {
				 $id = $request['record_id'];
				 $report = Horticuturedailyreport::findOrFail($id);
				 $resdate = date("Y-m-d", strtotime($redate));
				 $res_ar =  checkDailyStatus($resdate, $site );
				 if($res_ar[2] == 'yes'){}
				 else
			...
			else
			{
				...
				$insertcon = Horticuturedailyreport::create($request->all());
				$lastInsertedId= $insertcon->id;
				if(isset($edata['mloc']) && $edata['mloc']!="") 
				{
					foreach($edata['mloc'] as $mkey => $mwatering)
					{
						$locations = array();
					    foreach($edata['aloc'][$mkey] as $value)
						{
							$locations[] = $value;
						}
						if(count($locations)>0) $locs = implode(",",$locations);
						$mdiconsumptn = array("site"=>$edata['site'],'location'=>$locs,'manpower'=>$edata['mmanpower'][$mkey],'hrs'=>$edata['mwhrs'][$mkey],'report_id'=>$lastInsertedId,'type'=>'Watering','reporton'=>date("Y-m-d",strtotime($redate))); 
						$insertcon = HorticultureType::create($mdiconsumptn); 

					}
				}
				
				if(isset($edata['aname']) && $edata['aname']!="") 
				{
					foreach($edata['aname'] as $akey => $actname)
					{
						$locations = array();
					    foreach($edata['aloc'][$akey] as $value)
						{
							$locations[] = $value;
						}
						if(count($locations)>0) $locs = implode(",",$locations);
						$adiconsumptn = array("site"=>$edata['site'],'name'=>$edata['aname'][$akey],'location'=>$locs,'manpower'=>$edata['amanpower'][$akey],'hrs'=>$edata['awhrs'][$akey],'report_id'=>$lastInsertedId,'type'=>'Actvities','reporton'=>date("Y-m-d",strtotime($redate))); 
						$insertcon = HorticultureType::create($adiconsumptn); 
					}
				}
				
				
				if(isset($edata['apname']) && $edata['apname']!="") 
				{
					foreach($edata['apname'] as $apkey => $appname)
					{						
						$locations = array();
					    foreach($edata['aploc'][$apkey] as $value)
						{
							$locations[] = $value;
						}
						if(count($locations)>0) $locs = implode(",",$locations);
						$adiconsumptn = array("site"=>$edata['site'],'name'=>$edata['apname'][$apkey],'location'=>$locs,'fnumber'=>$edata['fnumber'][$apkey],'fdays'=>$edata['fdays'][$apkey],'report_id'=>$lastInsertedId,'type'=>'Applications','reporton'=>date("Y-m-d",strtotime($redate))); 
						$insertcon = HorticultureType::create($adiconsumptn);   
				...
				if(isset($edata['fname']) && $edata['fname']!="") 
				{
					foreach($edata['fname'] as $pkey => $pname)
					{
						$locations = array();
					    foreach($edata['floc'][$pkey] as $value)
						{
							$locations[] = $value;
						}
						if(count($locations)>0) $locs = implode(",",$locations);
						$pdiconsumptn = array("site"=>$edata['site'],'name'=>$edata['fname'][$pkey],'location'=>$locs,'qty'=>$edata['fqty'][$pkey],'report_id'=>$lastInsertedId,'type'=>'Pesticides ','reporton'=>date("Y-m-d",strtotime($redate))); 
						$insertcon = HorticultureType::create($pdiconsumptn);   
  ...
  $segment1 = Request::segment(1);
	...
	$sitearr =  \App\Sites::where('status', '=', '1')->orderBy('scode', 'ASC')->get()->pluck('name', 'id');
     ...
     foreach($fusers as $frr){
		 ...
		 $ch1 = curl_init('http://tra.bulksmshyderabad.co.in/websms/sendsms.aspx?userid=ZONUPT&password=tech12344&sender=ZONUPT&mobileno=9246698435,9948663172,9912111554,7997099090,9603347573,8499981218,7331105366,8340998383,9502826126,7331107655,8247429276,8499828842,9701221274,9848538337,9885387887,7331135201,7330788874,9502917979&msg='.$firesafetext.''); 

				 

				 $ch2 = curl_init('http://tra.bulksmshyderabad.co.in/websms/sendsms.aspx?userid=ZONUPT&password=tech12344&sender=ZONUPT&mobileno=9246698435,9948663172,9912111554,7997099090,9603347573,8499981218,7331105366,8340998383,9502826126,7331107655,8247429276,8499828842,9701221274,9848538337,9885387887,7331135201,7330788874,9502917979&msg='.$fmstext.''); 

				 

				 $ch3 = curl_init('http://tra.bulksmshyderabad.co.in/websms/sendsms.aspx?userid=ZONUPT&password=tech12344&sender=ZONUPT&mobileno=9246698435,9948663172,9912111554,7997099090,9603347573,8499981218,7331105366,8340998383,9502826126,7331107655,8247429276,8499828842,9701221274,9848538337,9885387887,7331135201,7330788874,9502917979&msg='.$pmstext.''); 

				 

				 $ch4 = curl_init('http://tra.bulksmshyderabad.co.in/websms/sendsms.aspx?userid=ZONUPT&password=tech12344&sender=ZONUPT&mobileno=9246698435,9948663172,9912111554,7997099090,9603347573,8499981218,7331105366,8340998383,9502826126,7331107655,8247429276,8499828842,9701221274,9848538337,9885387887,7331135201,7330788874,9502917979&msg='.$securitytext.'');  
...
echo 'http://tra.bulksmshyderabad.co.in/websms/sendsms.aspx?userid=ZONUPT&password=tech12344&sender=ZONUPT&mobileno=7207255435&msg='.$data.'';
 ...
 $relations = [
	...
	public function sentallemails(Request $request)
...

	{
		...
		$secure_cn = count($notented_ind['security']);

		if($secure_cn > 0){

		   $secure_string = implode(",",$notented_ind['security']);
	 ...
	 if($frs_cn > 0 || $fms_cn > 0 || $pms_cn > 0 || $secure_cn > 0) 

	 {

	 	

	 	if($firesafe_string!="") $firesafetext =  '<b>FIRE SAFETY</b> - Data Not Entered - '.$frs_cn.' sites ('.$firesafe_string.')'; else $firesafetext = "";

		if($fms_string!="") $fmstext =  '<b>FMS</b> - Data Not Entered - '.$fms_cn.' sites ('.$fms_string.')'; else $fmstext = "";

		if($pms_string!="") $pmstext =  '<b>PMS</b> - Data Not Entered - '.$pms_cn.' sites ('.$pms_string.')'; else $pmstext = "";

		if($secure_string!="") $securetext =  '<b>SECURITY</b> - Data Not Entered - '.$secure_cn.' sites ('.$secure_string.')';  else $securetext = "";
		...
		$email_array = [
            ...
            "emailSubject" => $date." APMS Daily Report Status",
			...
			"firesafetext" => $firesafetext,

			"securetext" => $securetext,

        ];

        Mail::send("emails.admin_notify", $email_array, function($m) use ($email_array) {
			...
			$emails = array("rrk@apmspl.com", "pvr@apmspl.com", "madhu-facilities@apmspl.com", "ambikaprasad@apmspl.com", "pavankumarpms@apmspl.com" , "pavankumarpms@apmspl.com", "naveenprasad-safety@apmspl.com", "safety@apmspl.com", "webportal@apmspl.com", "misreports@apmspl.com", "sereneparkpms@apmspl.com", "silveroakspms@apmspl.com","arunkumarreddy310@gmail.com","zonuptechnologies@gmail.com");
