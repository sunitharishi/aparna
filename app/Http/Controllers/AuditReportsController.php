...
public function getfmsdarauditreportsexport(Request $request)
			 ...
			 foreach ($results as $kk => $client) {
				if (count($client) > 0) {
					foreach ($client as $tt) {
					   ...
					   $D4 =  (int)$tt->pwr_tot_lt;
					   $E4 =  (int)$pwr_solarpwrunits;
  ...
  public function getpmswaterauditreports(Request $request)
	 ...
	 public function getpmswaterauditreportexport(Request $request)
	{
	  ...
	  if(Auth::user()->id == 1){
		...
		$paymentsArray = array();
		$paymentsArray[] = ['Site', 'Date', 'Time', 'Watering', 'Reason'];			
		if(count($results) > 0) {
		  foreach ($results as $kk => $client) {
			  if (count($client) > 0) {
				  foreach ($client as $tt)
				  {
					...
					$resrow['reson'] = $tt->land_water_qty_reson;
					$paymentsArray[] =  $resrow;
				 }
			    ...
			    foreach ($client as $tt){
					 ...
					 $grab_val = $tt->housekp_debr_garbage; 
									$resrow = array();
									 ...
									 $resrow['report_on'] =  date("d-m-Y", strtotime($tt->reporton)); 
									 $resrow['report_month'] =  date("F Y", strtotime($tt->reporton)); 
									 $resrow['occupancy'] =  (int)$tt['occupancy_asdate_owners']+(int)$tt['occupancy_asdate_tenants'];
									$resrow['clbhous_users_gym'] =  $tt['clbhous_users_gym'];
									$resrow['clbhous_shuttle'] =  $tt['clbhous_shuttle'];
									$resrow['clbhous_users_pool'] =  $tt['clbhous_users_pool'];
									$resrow['clbhous_users_tennis'] =  $tt['clbhous_users_tennis'];
 ...
 public function getsecage20Auditreportexport(Request $request)
    ...
    { 	
	     ...
	     if(Auth::user()->id == 1){
		  ...
		  $categories = \App\Snagcategory::orderBy('cattitle', 'ASC')->get()->pluck('cattitle', 'id')->prepend('ALL', '');
			...
			$startdate = $cur_year."-".$cur_month."-"."01";
			$siteIds = "";
			$siteCounts = count($siteattrname);
			$i=0;
			foreach($siteattrname as $kk => $site){
				$i++;
				if($i<$siteCounts) $siteIds .= $kk.",";
				else $siteIds .= $kk;				
			} 			
			$siteIds = explode(",",$siteIds);
			
			$audit_cn = Snagreport::whereIn('sid', $siteIds)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->count();
			   if($audit_cn > 0){
			    $audit_res = Snagreport::whereIn('sid', $siteIds)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->get();
			   $results[$kk] = $audit_res;
	 ...
	 public function eyesore(Request $request)
    { 	
		 //echo  Auth::user()->id;
		 //echo  Auth::user()->emp_id;
	     if(Auth::user()->id == 1){
		  ...
		  $sitenames = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('ALL', '');
			...
			$audit_cn = Eyesore::whereIn('sid', $siteIds)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->count();
			   if($audit_cn > 0){
			    $audit_res = Eyesore::whereIn('sid', $siteIds)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->get();
			   $results[$kk] = $audit_res;
		 ...
		 return view('eyesore.index', $relations);
    }
	
	
	public function snagsiteselect(Request $request)
    { 	
		  ...
		  $categories = \App\Snagcategory::orderBy('cattitle', 'ASC')->get()->pluck('cattitle', 'id')->prepend('ALL', '');
			...
			$startdate = $cur_year."-".$cur_month."-"."01";
			$i=0;	
			$audit_cn = Snagreport::where('sid', $siteID)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->count();
			   if($audit_cn > 0){
			    $audit_res = Snagreport::where('sid', $siteID)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->get();
			   $results[$siteID] = $audit_res;
			   }
		  $relations = [
			'sites' => $siteattrname,
			'sitenames' => $sitenames,
		 ...
		 return view('misreports.snagsiteselect', $relations);
    }
	
	
	
	public function soresiteselect(Request $request)
    { 	
		  ...
		  $sitenames = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('ALL', '');
			...
			$startdate = $cur_year."-".$cur_month."-"."01";
			$i=0;	
			$eyesore_cn = Eyesore::where('sid', $siteID)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->count();
			   if($eyesore_cn > 0){
			    $eyesore_res = Eyesore::where('sid', $siteID)->where('status', '=','1')
				->orWhere('status', '=', '2')
				->orderBy('reporton', 'DESC')
				->get();
			   $results[$siteID] = $eyesore_res;
			   }
		  $relations = [
			'sites' => $siteattrname,
			'sitenames' => $sitenames,
		 ...
		 return view('eyesore.siteselect', $relations);
    }
	...
	public function geteyesorecreate()
	{
		if(Auth::user()->id == 1){
		  $siteattrname = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');;
		  $sitenames = \App\Sites::where('status', '=', '1')->orderBy('name', 'ASC')->get()->pluck('name', 'id')->prepend('Please Select', '');;
		...
		return view('eyesore.create',$relations);
	}
   ...
   if ($edata['todate']){
	...
	// echo $siteCount;
	 ...
	 if(count($fst)>0)
	 {
		  ...
		  if(Auth::user()->id == 1)
		  {
		  	 ...
		  	 if($siteCount==0)
			 {
			 	if($catCount>0 && $EmptyTestArray!=1)
				{
					...
					->orWhere('status', '=', '2')->orderBy('reporton', 'DESC')->get();
					else
					$audit_res = Snagreport::select('snagreports.*')->Where($fst)->whereIn('cid', $cats)->where('status', '=','0')->orderBy('reporton', 'DESC')->get();
				}
				else
				{
					if($edata['status']==1)
					...
					$audit_res = Snagreport::select('snagreports.*')->Where($fst)->where('status', '=','0')->orderBy('reporton', 'DESC')->get();
				}
			 }
			 else
			 {
		  ...
		  else
		  {
			...
			else
			{
				...
				$audit_res = Snagreport::select('snagreports.*')->Where($fst)->where('status', '=','0')
				->orderBy('reporton', 'DESC')->get();
			}
		  }
	 }
	 else
	 {
		if($catCount>0 && $EmptyTestArray!=1)
		{
			...
			$audit_res = Snagreport::select('snagreports.*')->Where($fst)->whereIn('cid', $cats)->where('status', '=','0')
			->orderBy('reporton', 'DESC')->get();
		}
		else
		{
	 ...
	 if(count($audit_res)>0)
	 {
		 foreach($audit_res as $key=>$res)
		 {
				 ...
				 $year = date('Y',strtotime($res->timeline));
				 if($year!='1970' || $year!='0000' || $year!='0001' || $year!='-0001') $timeline = date("d-m-Y",strtotime($res->timeline));
				 else  $timeline = ""; 
				 $data[$key]['id'] = $res->id;
				 $data[$key]['site'] = $nsite;
				 $data[$key]['category'] = $ncat;
	...
	'catcount' => $catCount
	 ...
	 public function getsorereports(Request $request)
    { 	
	...
	if ($edata['fromdate']) {
	  $frm_Date = date("Y-m-d",strtotime($edata['fromdate']));		  
	   $match[] = "eyesores.reporton,>=,$frm_Date";
   ...
   if ($edata['todate']){
		$to_Date = date("Y-m-d",strtotime($edata['todate']));		  
		//$to_Date = date("Y-m-d",strtotime($edata['to_date']))." 23:59:59";			
		  $match[] = "eyesores.reporton,<=,$to_Date";
	} 
	
	if ($edata['site'] && $edata['site']!='') {
	   $siteCount=1;
	   $site = $edata['site']; 
	   $match[] = "eyesores.sid,=,$site";
	}/*
	...
	$sites = explode(",",$sites);
	if($edata['status']==1)
	$audit_res = Eyesore::select('eyesores.*')->Where($fst)->where('status', '=','1')
	->orWhere('status', '=', '2')
	->orderBy('reporton', 'DESC')->get();
	else
	$audit_res = Eyesore::select('eyesores.*')->Where($fst)->where('status', '=','0')
	->orderBy('reporton', 'DESC')->get();
	 ...
	 if(count($audit_res)>0)
	 {
		 foreach($audit_res as $key=>$res)
		 {
				 ...
				 $data[$key]['rectype'] = $res->reporton;
				 $data[$key]['recommendation'] = $res->recommendation;
				 $data[$key]['imagepath'] = $res->before_image;
				 $data[$key]['reporton'] = date("d-m-Y",strtotime($res->reporton));
				 $data[$key]['timeline'] = $timeline;
				 $data[$key]['status'] = $res->status;
		 } 
	...
	exit;*/
	 ...
	 return view('eyesore.soredetail', $relations);
 	}	
	 ...
	 else
	 {
		 ...
		 if(count($audit_res)>0)
		 {
			 foreach($audit_res as $key=>$res)
			 {
					 ...
					 $timelineYear = str_replace("--","-",$res->timeline);					 
					 $year = date('Y',strtotime($timelineYear));
					 if($year=='1970' || $year=='0000' || $year=='0001' || $year=='-0001') $timeline = "";
					 else   $timeline = date("d-m-Y",strtotime($res->timeline));
					 $data[$key]['i'] = $i;
		...
		$excel->sheet('sheet1', function($sheet) use ($paymentsArray) {	
				...
				for ($i = 1; $i < count($paymentsArray); $i++) {
					...
					if($image2!="" && file_exists($image))
					{
						...
						$sheet->getStyle('F'.$j)->getAlignment()->setWrapText(false);
	...
	public function getsoreexport(Request $request)
     {
		$edata = $request->all();
		 $siteCount=0;
		 $catCount = 0;
		 ...
		 // echo "<pre>"; print_r($fst); echo "</pre>";
		  if(Auth::user()->id == 1)
		  {
			...
			->orWhere('status', '=', '2')->get();
			 ...
			 $audit_res =  Eyesore::select('eyesores.*')->Where($fst)->where('status', '=','0')->get();
		  }
		  else
		  {
		  	  if($edata['status']==1)
	 ...
	 {
	 	  if($edata['status']==1)
	 	  $audit_res = Eyesore::select('eyesores.*')->where('status', '=','1')->orWhere('status', '=', '2')->get();
		  else
		  $audit_res = Eyesore::select('eyesores.*')->where('status', '=','0')->get();
	 }
	 
		 $paymentsArray = array();
	  	 $paymentsArray[] = ['S.No', 'Site', 'Description', 'Location', 'Beofre Image', 'After Image','Start Date','End Date','Status'];
		 $nsite = "";
		 $psite = "";
		 $scount=0;
		 $i = 0;
		 ...
		 if(count($audit_res)>0)
		 {
			 foreach($audit_res as $key=>$res)
			 {
					 ...
					 $data[$key]['observation'] = $res->observation;
					 $data[$key]['location'] = $res->location;
					 $data[$key]['bimagepath'] = $res->before_image;
					 $data[$key]['aimagepath'] = '';
					 $data[$key]['reporton'] = date("d-m-Y",strtotime($res->reporton));
					 $data[$key]['timeline'] = $timeline;
					 $data[$key]['status'] = $status;
		...
		Excel::create('Eyesore_Report', function($excel) use ($paymentsArray) {
		$excel->getDefaultStyle()
		->getAlignment()
		->setVertical(\PHPExcel_Style_Alignment::VERTICAL_TOP);
	
		// Set the spreadsheet title, creator, and description
		$excel->setTitle('Eyesore_Report');
		$excel->setCreator('Laravel')->setCompany('Aparna');
		$excel->setDescription('sorereportsubmission file');
		//echo "<pre>"; print_r($paymentsArray); echo "</pre>";
		// Build the spreadsheet, passing in the payments array
		$excel->sheet('sheet1', function($sheet) use ($paymentsArray) {	
				...
				for ($i = 1; $i < count($paymentsArray); $i++) {
					...
					$maxCell = 'E'; 			 	
					$sheet->setCellValue($maxCell.$j, "");  					
					$image2 = $paymentsArray[$i]['bimagepath'];
					$image = public_path('uploads/sore/thumb/'.$image2);
					if($image2!="" && file_exists($image))
					{
						$objDrawing = new PHPExcel_Worksheet_Drawing;
						$objDrawing->setPath($image); //your image path
					...
					/*$maxCell2 = 'F'; 			 	
					$sheet->setCellValue($maxCell2.$j, "");
					
					$image3 = $paymentsArray[$i]['aimagepath'];
					$imageb = public_path('uploads/sore/thumb/'.$image3);
					if($image3!="" && file_exists($imageb))
					{
						$objDrawing = new PHPExcel_Worksheet_Drawing;
						$objDrawing->setPath($imageb); //your image path
						$objDrawing->setResizeProportional(false);
						$objDrawing->setWidthAndHeight(150,150);
						$objDrawing->setCoordinates($maxCell2.$j);
						$sheet->setSize(array(
							'F'.$j => array(
								'width'     => 21,
								'height'    => 110
					...
					$sheet->getStyle('C'.$j)->getAlignment()->setWrapText(true);
					$sheet->getStyle('D'.$j)->getAlignment()->setWrapText(true);
					$sheet->getStyle('E'.$j)->getAlignment()->setWrapText(false);
					$sheet->getStyle('F'.$j)->getAlignment()->setWrapText(false);
					$sheet->getStyle('G'.$j)->getAlignment()->setWrapText(true);
					$sheet->getStyle('H'.$j)->getAlignment()->setWrapText(true);
					$sheet->getStyle('I'.$j)->getAlignment()->setWrapText(true);
	...
	public function fileStore(Request $request)
    {
       ...
       $fileName = uniqid().'_snag.'.request()->imagepath->getClientOriginalExtension();
       $request->imagepath->move(public_path('uploads/snag/'), $fileName);
	   $w=150; 
	   $h=150; 
	   $filename_extension1 = public_path().'/uploads/snag/'.$fileName;
	   $newfilename= public_path().'/uploads/snag/thumb/'.$fileName;
	   resize_byratio($filename_extension1, $w, $h, $newfilename);				
	   $imagepath = $fileName;
	   $store_val  = array('imagepath'=>$imagepath);
	   $report = Snagreport::where('id', '=',  $request->snag_id)->update($store_val);	
       return response()->json(['path'=>$fileName, 'type'=>'snagupload']);		
    }
	public function rfileStore(Request $request)
    {
        $request->validate([
            'recimagepath' => 'required',
        ]);				
       $fileName = uniqid().'_rec.'.request()->recimagepath->getClientOriginalExtension();
       $request->recimagepath->move(public_path('uploads/snag/rec/'), $fileName);
	   $w=150; 
	   $h=150; 
	   $filename_extension1 = public_path().'/uploads/snag/rec/'.$fileName;
	   $newfilename= public_path().'/uploads/snag/rec/thumb/'.$fileName;
	   resize_byratio($filename_extension1, $w, $h, $newfilename);				
	   $imagepath = $fileName;
	   $store_val  = array('recimagepath'=>$imagepath);
	   $report = Snagreport::where('id', '=',  $request->snag_id)->update($store_val);	
       return response()->json(['path'=>$fileName, 'type'=>'rec']);				
    }
	...
	public function storesnag(StoreSnagRequest $request)
	{
		...
		$recimagepath  = "";
		$recommendation = "";
		if(isset($edata['catid']) && $edata['catid']!="") $catid = $edata['catid']; else $catid = "";		
		...
		if(isset($edata['remarks']) && $edata['remarks']!="") $remarks = $edata['remarks']; else $remarks = "";	
		 ...
		 if($rectype=='image')
		 {
		 	 $snag_array =  array("cid"=>$catid, "uid"=>$uid, "observation"=>$observation, "location"=> $location, "reporton"=> $reporton, "timeline"=> $timeline, "status"=> $status, "rectype"=> $rectype, "remarks"=> $remarks);
		 }	
		 else 
		 {
		 	  $snag_array =  array("cid"=>$catid, "uid"=>$uid, "observation"=>$observation, "location"=> $location, "reporton"=> $reporton, "timeline"=> $timeline, "status"=> $status, "rectype"=> $rectype, "recommendation"=> $recommendation, "remarks"=> $remarks);
		 }
	   ...
	   if($report)
	   {
	   		$audit_res =  Snagreport::select('snagreports.*', 'snagcategories.cattitle', 'sites.name')->leftJoin('snagcategories','snagcategories.id','=','snagreports.cid')->leftJoin('sites','sites.id','=','snagreports.sid')->Where('snagreports.id', '=', $edata['id'])->first();
			$site = $audit_res['name'];
			$category = $audit_res['cattitle'];
			$observation = $audit_res['observation'];
			$location = $audit_res['location'];
			$reporton = date("d-m-Y",strtotime($reporton));
			$rectype = $audit_res['rectype'];
			
			if($status ==1) $status = "Open";
			else  if($status ==2) $status = "Inprogress";
			else  if($status ==0) $status = "Closed";
			
			if($edata['snag_image']) $snagimage = '<div id="image_pic"><a href="/uploads/snag/'.$audit_res['imagepath'].'" style="width:500px;" class="open-lightbox"><img src="/uploads/snag/thumb/'.$audit_res['imagepath'].'" class="img-responsive" style="max-width:150px; width:150px; height:150px;"></a><div style="position: absolute;margin-left: 127px;margin-top: -150px;"><a href="javascript:void(0)" onclick="imgDelete('.$audit_res['id'].');" style="color: #FF0000;font-size: 22px;font-weight: bold;">X</a></div></div>';
			
			if($rectype=='image') $recommendation = '<div id="rimage_pic"><a href="/uploads/snag/rec/'.$audit_res['recimagepath'].'" style="width:500px;" class="open-lightbox"><img src="/uploads/snag/rec/'.$audit_res['recimagepath'].'" class="img-responsive" style="max-width:150px; width:150px; height:150px;"></a><div style="position: absolute;margin-left: 127px;margin-top: -150px;"><a href="javascript:void(0)" onclick="recimgDelete('.$audit_res['id'].');" style="color: #FF0000;font-size: 22px;font-weight: bold;">X</a></div></div>';			
			else if($rectype=='text') $recommendation = $edata['recommendations'];
			else $recommendation = "";
			
			$year = date('Y',strtotime($timeline));	
			if($year =='1970' || $year=='0000' || $year=='0001' || $year=='-0001') $timeline2 = "";
			else  $timeline2 = date("d-m-Y",strtotime($timeline)); 
			
			
			$result = array("category"=>$category, "observation"=>$observation, "location"=>$location, "snagimage"=>$snagimage, "status"=>$status, "reporton"=>$reporton, "timeline"=>$timeline2, "rectype"=>$rectype, "recommendation"=>$recommendation);
	   }
	...
	public function submitsore(StoreSnagRequest $request)
	{
		...
		if(isset($edata['timeline']) && $edata['timeline']!="") $timeline = date('Y-m-d', strtotime($edata['timeline'])); else $timeline = "";
		if(isset($edata['status']) && $edata['status']!="") $status = $edata['status']; else $status = 1;
		if(isset($edata['before_image'])) 		
		{
			$filename = uniqid();			
			$req = $edata['before_image'];
			$extension = $req->getClientOriginalExtension();
			$file = $req->move('uploads/sore/',$filename."_bsore.".$extension);
			$filename_extension = $filename."_bsore.".$extension;
			$w=150; 
			$h=150; 
			$filename_extension1 = public_path().'/uploads/sore/'.$filename.'_bsore.'.$extension;
			$newfilename= public_path().'/uploads/sore/thumb/'.$filename.'_bsore.'.$extension;
			resize_byratio($filename_extension1, $w, $h, $newfilename);				
			$bimagepath = $filename_extension;
		}
		else $bimagepath = "";
		
		if(isset($edata['remarks']) && $edata['remarks']!="") $remarks = $edata['remarks']; else $remarks = "";			
		$uid = Auth::user()->id;
		 $sore_array =  array("uid"=>$uid, "sid"=>$site, "observation"=>$observation, "location"=> $location, "reporton"=> $reporton, "status"=> $status, "before_image"=> $bimagepath,  "remarks"=> $remarks);	
	   Eyesore::create($sore_array);
	   return redirect()->route('eyesore');
	}
