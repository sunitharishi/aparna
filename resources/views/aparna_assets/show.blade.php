  ...
  color: #fff;
    ...
    <div class="panel panel-default conummity-heading community-assets-view-page">
        <div class="panel-heading">
            Aparna Asset<pre style="display:none;"></pre>
            <p class="pull-right">
                <a href="javascript:void(0)" class="btn aturl green-back" data-toggle="modal" data-target="#summary2">Maintenance</a>
				<a href="{{ route('acommbreakdown.edit', $comm_asset->id) }}" class="btn green-back">Break Down</a>
				<a href="{{ route('acommincident.edit', $comm_asset->id) }}" class="btn green-back">Incident</a>
				<a href="{{ route('acommjobcard.edit', $comm_asset->id) }}" class="btn green-back">Job Card</a>
				<a href="{{ route('aparnaassets.index') }}" class="btn green-back">Back</a>
            </p>
        </div>
...
<script type="text/javascript">
		...
		function EditComAsset(id)
		{
			...
			type: "get",
			cache:false,
			url: '{{ route('aparnaassets.cassetedit') }}',
			data: { cassetId: id},
			dataType: 'json',
		...
		function submitMaintenance2()
		{
			...
			$.ajax({
				type: "get",
				cache:false,
				url: '{{ route('aparnaassets.maintenance') }}',
				data: { asset_id: asset_id, amc_type: amc_type, sfromdate: sfromdate, stodate: stodate},
				success: function( response ) 
				{
					if(response=="Success")
					{
						...
						location.replace("/aparnaassets/"+asset_id);
					}
				}  
			 });
		 }
