...
<style type="text/css">
    ...
    {!! Form::open(['method' => 'POST', 'route' => ['jacommobcard.save'],'id'=>'frmJobcard','onsubmit'=>'return saveJobcard()']) !!}
        ...
        <div class="panel-body jobacard nobcardds">
            <div class="row class-ctration">
               <!-- @if($userid==1)
				...
				<input type="hidden" name="asset_id" id="ast_id" value="<?php echo $casset->id; ?>">
                <div class="col-sm-2 form-group inreponsibleee">
                    {!! Form::label('jctype', 'Job card type', ['class' => 'control-label']) !!}
                    {!! Form::select('jctype', $jbtypes, old('jctype')?old('jctype'):(isset($jobcard)?$jobcard->jctype:''), ['class' => 'form-control select2mes erequired','onchange'=>'setJobtypecom(this)']) !!}
                    <p class="help-block"></p>
                ...
                <div class="col-sm-3 form-group sparess jctypeoptions1 breadksoen1" <?php if($jctype<>'1' && $jctype<>'2') echo ' style="display: none;"';?>>
                    ...
                    {!! Form::select('jctype12', $jcoptions, $jctype12, ['class' => 'form-control select2mes','onchange'=>'getJobCardEmployees(this)']) !!}
                    @endif
                </div>
                     <input type="hidden" name="category_id" id="category_id" value="<?php echo $casset->category_id; ?>">
					<!-- <input type="hidden" name="asset_id" id="asset_id" value="<?php //echo $casset->id; ?>">-->
                ...
                <div class="col-sm-2 form-group incident-idb">
                    {!! Form::label('spares', 'Spares', ['class' => 'control-label']) !!}
                    {!! Form::select('spares', $sparestypes, old('spares')?old('spares'):(isset($jobcard)?$jobcard->spares:''), ['class' => 'form-control select2mes erequired']) !!}
                    <p class="help-block"></p>
    ...
    <div class="btn-save community-jobcard-edit"> {!! Form::submit('Submit', ['class' => 'btn btn-danger btn-submitt']) !!}
