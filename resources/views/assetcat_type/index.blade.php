...
@section('content')
   <!-- <h3 class="page-title">Categories</h3>-->
    ...
    <div class="panel panel-default respositroy-view">
        <div class="panel-heading assetcat-type-heading">
            Asset Category Types
             <p class="ptag">
       <a href="{{ route('assetcat-types.create') }}" class="btn btn-new">Add</a>
                    ...
                    @if (count($category) > 0)
                        @foreach ($category as $client_status)
                            <tr data-entry-id="{{ $client_status->id }}">
                                <td></td>
                                 <td>{{ $client_status->asstypename }}</td>
                                <td>{{ $client_status->name }}</td>
								<td>{{ $client_status->ccode }}</td>
                                <td>{{ $client_status->prefix }}</td>
                                <td><a href="{{ route('assetcat-types.edit',[$client_status->id]) }}" class="btn btn-xs btn-inverse">Edit</a>{!! Form::open(array(
                'style' => 'display: inline-block;',
                'method' => 'DELETE',
                'onsubmit' => "return confirm('".trans("Are you sure?")."');",
                'route' => ['assetcat-types.destroy', $client_status->id])) !!}
