   ...
   <!-- <h3 class="page-title">Categories</h3>-->
    ...
    <div class="panel panel-default respositroy-view category-frontpage">
        <div class="panel-heading">
            Categories
             <p class="ptag">
       <a href="{{ route('category.create') }}" class="btn btn-new">Add</a>
    </p>
        </div>

        <div class="panel-body category-section">
                    ...
                    @if (count($category) > 0)
                        @foreach ($category as $client_status)
                            <tr data-entry-id="{{ $client_status->id }}">
                                ...
                                <td><a href="{{ route('category.edit',[$client_status->id]) }}" class="btn btn-xs btn-inverse">Edit</a>{!! Form::open(array(
                'style' => 'display: inline-block;',
                'method' => 'DELETE',
                'onsubmit' => "return confirm('".trans("Are you sure?")."');",
                'route' => ['category.destroy', $client_status->id])) !!}
