...
<?php
 ...
 ?><?php */?>
    ...
    <div class="count-calender">
        <div class="row">
            <div class="col-xs-12 dashboard-calendar">
				  ...
				  foreach( $assets as $ke => $asset) {  ?>
                   {
                      ...
                      "start":'<?php $da = $asset['created_at']; $datearr = explode(" ",$da); echo $datearr[0];?>',
                      "end":'<?php $da = $asset['created_at']; $datearr = explode(" ",$da); echo $datearr[0];?>',
                      "url":"/tasks/<?php echo $asset['id'] ?>"
                   },
                   <?php } echo $comassets_events;?>
                ]
            });
        });
		
   /* function getCalView(val){
	  alert(val);  
	  var site = val;
	   $.ajax({ 
				type: "get",
				cache:false,
				dataType: "json",
				url: '{{ route('contactus.getSitecalendarview') }}',
				data: { site: site },
				success: function( response ) {
					//alert(response["assets"]);
					//alert(response["comassets_events"]);
