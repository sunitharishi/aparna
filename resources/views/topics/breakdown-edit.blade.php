...
@section('content')
    ...
    {!! Form::open(['method' => 'POST', 'route' => ['breakdown.save'],'id'=>'frmBreakdown']) !!}
        ...
        <div class="panel-body breakdown-car">
            <div class="row class-ctration">
                ...
                <div class="col-sm-2 form-group community-breaksoen">
                    ...
                    @if(isset($breakdown))
                    {!! Form::select('site_id', $communities, old('site_id')?old('site_id'):(isset($breakdown)?$breakdown->site_id:''), ['class' => 'form-control select2mes erequired','onchange'=>'getTopicsCommunityAssets()','disabled'=>'disabled']) !!}
                     @else 
                ...
                <div class="col-sm-2 form-group breakdown-categoryund">
                    ...
                    @if(isset($breakdown))
                    {!! Form::select('category_id', $categories, old('category_id')?old('category_id'):(isset($comasset)?$comasset->category_id:$breakdown->category_id), ['class' => 'form-control select2mes erequired','onchange'=>'getTopicsCommunityAssets()','disabled'=>'disabled']) !!}
                    @else 
                ...
                <div class="col-sm-2 form-group breakdown-categoryund" id="subcatdiv">
                    {!! Form::label('sub_category_id', 'Sub Category', ['class' => 'control-label']) !!} <br />
                    @if(isset($breakdown))
                    {!! Form::select('sub_category_id', $subcategories, old('sub_category_id')?old('sub_category_id'):(isset($breakdown)?$breakdown->sub_category_id:''), ['class' => 'form-control select2mes erequired','disabled'=>'disabled']) !!}
                    @else 
                    {!! Form::select('sub_category_id', $subcategories, old('sub_category_id')?old('sub_category_id'):(isset($breakdown)?$breakdown->sub_category_id:''), ['class' => 'form-control select2mes erequired']) !!}
                    @endif
                    <p class="help-block"></p>
                ...
                <div class="col-sm-3 form-group community-assoeknf" id="topics_assets">
                    ...
                    <br>
                    <select name="asset_id" id="asset_id" class="form-control select2mes" onchange="getAssetEmployees(this)" disabled="disabled">
                       ...
                       @if($other_assets) 
                        ...
                        <option value="{{ $oasset->id }}" @if($jctype12==$oasset->id) selected="selected" @endif >{{ $oasset->code }}-{{ $oasset->name }}</option>
                        @endforeach
                       @endif 
                    </select>
                    <p class="help-block"></p>
    ...
    <div class="btn-save btn-breakdownedit-submit"> {!! Form::submit('Submit', ['class' => 'btn btn-danger btn-submitt']) !!}  
