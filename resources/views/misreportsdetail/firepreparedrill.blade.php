...
<div class="misback-button"><a href="/misreportsoptions/<?php echo $year."/".$month ?>">Back</a></div>
    ...
    <h3 class="page-title req" style="text-align:center;">Fire Safety Preparedness & Fire Alarm Operation Training</h3>
{!! Form::open(['files' => 'true','route' => 'uploadexcel.savefireprepare','role' => 'form', 'class'=>'form-inline upload-attendance-form', 'onsubmit' =>"return subform()"]) !!}
	...
	<div class="row">
				...
				<?php /*?> {!! Form::label('dailycat', 'Community', ['class' => 'control-label']) !!}
					...
					<?php /*?><div class="laggedtime"><label class="conducterd-label">Conducted Date: </label><input type="text" id="date_of_conducted" name="date_of_conducted" value="<?php  echo date("d-m-Y"); ?>" class="hasDatePicker form-control"></div> <?php */?>
	 ...
	 var month = $("#month").val();
	  ...
	  if(val == "") {	  
		...
		$.ajax({
			type: "get",
			cache:false,
			url: '{{ route('validation.getfiresafecount') }}',
